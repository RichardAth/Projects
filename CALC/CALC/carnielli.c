...
/*
* Walter Carnielli's generalized 3x+1 function.
...
/* the cycle starting with *Aptr is printed */
{
	...
	strcpy(buff, "carnielli_cycle.out");
	B = COPYI(Aptr);
	MIN_ELT = COPYI(Aptr);
	do {
		Temp = B;
...
* Z[0],..., Z[c]  if and only if CARNIELLI_IS_CYCLE() = 1.
*/
{
	USL k;
	MPI *B, *Temp;
		...
		if (FLAG[n] > n) {
			...
			if (t) {
				if (c == 0) {
					...
					fprintf(outfile, "\ncycle %u:\n", c + 1);
					fclose(outfile);*/
					CARNIELLI_P_CYCLE(A, d, c + 1, verbose);
					c++;
				}
				else {
					/* store start of subsequent cycles */
					s = CARNIELLI_IS_CYCLE(A, d, Z, c);
					if (s == 0)
						FREEMPI(A);
					else
					{
...
void CARNIELLI_CYCLEX()
{
	...
	for (d = m; d <= n; d++) {
		...
		fclose(outfile);
		cycle_count = CARNIELLI_CYCLE(d, infty, RANGE, verbose);
		outfile = fopen(buff, "a");
		if (cycle_count == d + 1) {
