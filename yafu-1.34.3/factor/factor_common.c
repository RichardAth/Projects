...
void init_factobj(fact_obj_t *fobj)
{
	...
	strcpy_s(fobj->flogname, sizeof(fobj->flogname), "factor.log");
	fobj->do_logging = 1;
	...
	strcpy_s(fobj->qs_obj.siqs_savefile, sizeof(fobj->qs_obj.siqs_savefile), "siqs.dat");
	init_lehman();
	...
	strcpy_s(fobj->nfs_obj.outputfile, sizeof(fobj->nfs_obj.outputfile), 
		"nfs.dat");			//default
	strcpy_s(fobj->nfs_obj.logfile, sizeof(fobj->nfs_obj.logfile), "nfs.log");			//default
	strcpy_s(fobj->nfs_obj.fbfile, sizeof(fobj->nfs_obj.fbfile), "nfs.fb");				//default
	...
	*fobj->nfs_obj.filearg = '\0';
	...
	strcpy_s(fobj->autofact_obj.plan_str, sizeof(fobj->autofact_obj.plan_str), 
		"normal");
...
void alloc_factobj(fact_obj_t *fobj)
{
	...
	mallocCheck(fobj->rho_obj.polynomials)
	...
	mallocCheck(fobj->fobj_factors)
	for (uint32 i=0; i < fobj->allocated_factors; i++)
	{
...
void add_to_factor_list(fact_obj_t *fobj, mpz_t n)
{
	...
	if (gmp_base10(n) <= fobj->aprcl_prove_cutoff) /* prove primality of numbers <= aprcl_prove_cutoff digits */
	{
		...
		if (Vflag > 0)
			avFlag = (gmp_base10(n) < fobj->aprcl_display_cutoff) ? APRTCLE_VERBOSE0 : APRTCLE_VERBOSE1;
		else
			avFlag = APRTCLE_VERBOSE0;

		if (avFlag == APRTCLE_VERBOSE1)
			printf("\n");
		ret = mpz_aprtcle(n, avFlag);
		if (avFlag == APRTCLE_VERBOSE1)
			printf("\n");
...
int check_if_done(fact_obj_t *fobj, mpz_t N)
{
	...
	for (uint32 i=0; i<fobj->num_factors; i++)
	{		
		int j;
		for (j=0; j<fobj->fobj_factors[i].count; j++)
			mpz_mul(tmp, tmp, fobj->fobj_factors[i].factor);
			...
			for (uint32 i=0; i<fobj->num_factors; i++)
			{
				if (!mpz_probab_prime_p(fobj->fobj_factors[i].factor, NUM_WITNESSES))
				{
					...
					else
					{
						...
						for (uint32 j=0; j< fobj_refactor->num_factors; j++)
						{
							int k;
...
void factor(fact_obj_t *fobj)
{
	...
	if (mpz_perfect_power_p(b))
	{
		...
		if (flog != NULL)
		{
			...
			{
				for (int k=0; k<fobj->fobj_factors[j].count; k++)
				{
...
void spfactorlist(uint64 nstart, uint64 nrange)
{
	...
	fact_obj_t f;
	uint64 sf;
	uint64 tflimit = (uint64)sqrt(sqrt((double)nstart + nrange));
