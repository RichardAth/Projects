...
uint64 sp_shanks_loop(mpz_t N, fact_obj_t *fobj)
{
	...
	for (i=NUM_SQUFOF_MULT-1;i>=0;i--)
	{
		...
		mult_save[i].b0 = (uint32)mpz_get_ui(gmptmp);
...
uint64 LehmanFactor(uint64 N, double Tune, int DoTrial, double CutFrac)
{
	...
	B = (uint32)(Tune * (1.0+(double)mpz_get_ui(tmpz)));

	FirstCut = (uint32)(CutFrac*B);
	...
	for(k=1; k<=Bred; k++)
	{
		...
		x *= 2;
		a = (int64)(x+0.9999999665); //very carefully chosen.
		//Let me repeat that: a = x+0.9999999665.  Really.
		b=a%inc;  
		b = (uint32)(a + (inc+r-b)%inc);   //b is a but adjusted upward to make b%inc=r.
