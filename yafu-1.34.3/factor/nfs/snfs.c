...
void print_snfs(snfs_t *poly, FILE *out)
{
	...
	char c, side[80];
	int d = (int)poly->difficulty; // round double to int
...
void approx_norms(snfs_t *poly)
{
	...
	eval_poly(res, (int64)a, (uint32)b, &poly->poly->alg);
	poly->anorm = mpz_get_d(res);

	eval_poly(res, (int64)a, (uint32)b, &poly->poly->rat);
...
#pragma warning (suppress: 4244)
		...
		if (oom_skew >= 6)
		{
			...
			job->mfbr = (uint32)((double)job->lpbr*2.9);
			job->rlambda = 3.6;
		}
...
#pragma warning (suppress: 4244)
		...
		{
			// for really really big skew, use 3 large primes
			job->mfba = (uint32)((double)job->lpba*2.9);
			job->alambda = 3.6;
		}
...
snfs_t* gen_brent_poly(fact_obj_t *fobj, snfs_t *poly, int* npolys)
{
	...
	if (poly->exp1 % 15 == 0 && (poly->coeff1 == 1) && (abs(poly->coeff2) == 1))
	{
		...
		mpz_pow_ui(polys->poly->m, m, (long long)k);
		polys->poly->skew = 1.;
	...
	else 
	{
		...
		for (i=start_deg; i<=stop_deg; i++)
		{
			...
			if (e % i == 0)
			{
				...
				polys[npoly].c[i] /= gcd64(cd, c0);
				polys[npoly].c[0] /= gcd64(cd, c0);
	...
	if (algebraic)
	{
		if (halved)
		{
			if (poly->form_type == SNFS_H_CUNNINGHAM)
			{
				...
				mpz_pow_ui(polys->poly->rat.coeff[1], polys->poly->rat.coeff[1], (long long)k);
				mpz_mul(polys->poly->rat.coeff[0], polys->poly->m, polys->poly->m);		// a1^(2k)
				mpz_set_ui(n, poly->base2);
				mpz_pow_ui(n, n, (long long)(2*k));
	...
	for (base = 0; base < 2; base++)
	{
		...
		for (deg=4; deg<7; deg++)
		{
			...
			else
			{
				...
				skew = pow((double)llabs(c0)/(double)cd, 1.0/(double)deg);
				mpz_set(polys[npoly].n, poly->n);
				polys[npoly].difficulty = d;
				polys[npoly].poly->skew = skew;
				...
				skew = pow((double)llabs(c0)/(double)cd, 1./(double)deg);
				// leading coefficient contributes to the difficulty
				//d += log10((double)cd);
				mpz_set(polys[npoly].n, poly->n);
				polys[npoly].difficulty = d;
				...
				if (numf > 1)
				{
					...
					for (j=0; j<numf; j++)
					{
						...
						// move it up
						i1 = (deg - e % deg);
						c0 = (int64)(pow((double)f[j], i1) * poly->coeff2);
						// since we moved the current factor up, move the other factors down
						// otherwise this would be the same as moving the whole composite base up.
						// also move the second term up...
						cd = (int64)(pow((double)b2, i1) * poly->coeff1);
						bb = 1;
						i2 = e % deg;
						for (k=0; k<numf; k++)	{
							if (k == j) 
...
#pragma warning (suppress: 4244)
						...
						// move it down
						i1 = e % deg;
						cd = (int64)(pow((double)f[j], i1) * poly->coeff1);
						// since we moved the current factor down, move the other factors up
						// otherwise this would be the same as moving the whole composite base down.
						c0 = (int64)(pow((double)b2, i1) * poly->coeff2);
						bb = 1;
						i2 = (deg - e % deg);
						for (k=0; k<numf; k++)
						{
	...
	for (i=0; i<nump1; i++)
	{
		...
		for (j=0; j<nump2; j++)
		{
			...
			if (c0 > cd)
			{
				...
				final_polys[npoly].c[deg] = cd / gcd64(c0, cd);
				final_polys[npoly].c[0] = c0 / gcd64(c0, cd);
				final_polys[npoly].poly->skew = pow(
					(double)llabs(final_polys[npoly].c[0])/(double)final_polys[npoly].c[deg], 1./(double)deg);
			...
			{
				mpz_set(final_polys[npoly].n, poly->n);					
				final_polys[npoly].c[deg] = c0 / gcd64(c0, cd);
				final_polys[npoly].c[0] = cd / gcd64(c0, cd);
				final_polys[npoly].poly->skew = pow(
					(double)llabs(final_polys[npoly].c[0])/(double)final_polys[npoly].c[deg], 1./(double)deg);
				final_polys[npoly].poly->alg.degree = deg;
				final_polys[npoly].difficulty = log(pow(2.71828, p1->difficulty) + pow(2.71828, p2->difficulty));
