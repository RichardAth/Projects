...
void do_sieving(fact_obj_t *fobj, nfs_job_t *job)
{
	...
	for (i=0; i<THREADS; i++)
	{
		...
		if (fobj->nfs_obj.rangeq > 0)
			thread_data[i].job.qrange = (uint32)ceil((double)fobj->nfs_obj.rangeq / (double)THREADS);
		else
			thread_data[i].job.qrange = (uint32)ceil((double)job->qrange / (double)THREADS);
...
void *lasieve_launcher(void *ptr)
{
	...
	sprintf(syscmd,"%s%s -%c %s -f %u -c %u -o %s -n %d",
		...
		*side,       // hehe (*side == side[0]) generate -a or -r
		...
		thread_data->job.qrange,   // -c <qrange value> (for (sieve_count)
