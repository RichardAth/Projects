...
void nfs(fact_obj_t *fobj)
{
	...
	if ((uint32)gmp_base10(fobj->nfs_obj.gmp_n) < fobj->nfs_obj.min_digits)
	{
	...
	while (!process_done)
	{
		...
		case NFS_STATE_INIT:
			...
			if (fobj->nfs_obj.rangeq > 0)
				job.qrange = (uint32)(ceil((double)fobj->nfs_obj.rangeq / (double)THREADS));
		...
		case NFS_STATE_FILTER:
			...
			else
			{
				...
				if (job.current_rels > job.min_rels)
					job.min_rels = (uint32)(job.current_rels * fobj->nfs_obj.filter_min_rels_nudge);
				else
					// no warning for conversion from double to int
				...
				else
				{
					...
					if ((t = fopen(tmpstr, "r")) == NULL)
					{
						...
						else
						{
							...
							job.use_max_rels = (uint32)(job.min_rels * 1.5);
...
int check_for_sievers(fact_obj_t *fobj, int revert_to_siqs)
{
		...
		(fobj->nfs_obj.nfs_phases & NFS_PHASE_SIEVE))
	{
		...
		{
			printf("WARNING: could not find ggnfs sievers %s, reverting to siqs!\n", name);
...
void get_ggnfs_params(fact_obj_t *fobj, nfs_job_t *job)
{
	...
	for (i=0; i<GGNFS_TABLE_ROWS - 1; i++)
	{
		if (d > ggnfs_table[i][0] && d <= ggnfs_table[i+1][0])
		{
			...
			//interp
			fblim = (uint32)(ggnfs_table[i+1][1] - 
				(scale * (double)(ggnfs_table[i+1][1] - ggnfs_table[i][1])));
			if (job->rlim == 0) job->rlim = fblim;
			if (job->alim == 0) job->alim = fblim;

			//pick closest entry
			if ((d - ggnfs_table[i][0]) < (ggnfs_table[i+1][0] - d))
				lpb = (uint32)ggnfs_table[i][2];
			else
				lpb = (uint32)ggnfs_table[i+1][2];
			if (job->lpbr == 0) job->lpbr = lpb;
			if (job->lpba == 0) job->lpba = lpb;

			//pick closest entry
			if ((d - ggnfs_table[i][0]) < (ggnfs_table[i+1][0] - d))
				mfb = (uint32)ggnfs_table[i][3];
			else
				mfb = (uint32)ggnfs_table[i+1][3];
	...
	if (found == 0)
	{
		...
		{
			fblim = (uint32)ggnfs_table[0][1];
			lpb = (uint32)ggnfs_table[0][2];
			mfb = (uint32)ggnfs_table[0][3];
			lambda = ggnfs_table[0][4];
			siever = (uint32)ggnfs_table[0][5];
			if (job->rlim == 0) job->rlim = fblim;
			if (job->alim == 0) job->alim = fblim;
			if (job->lpbr == 0) job->lpbr = lpb;
			if (job->lpba == 0) job->lpba = lpb;
			...
			if (job->alambda == 0) job->alambda = lambda;
			if (fobj->nfs_obj.siever == 0) fobj->nfs_obj.siever = siever;
			job->qrange = (uint32)ggnfs_table[0][7];
		}
		else
		{
			fblim = (uint32)ggnfs_table[GGNFS_TABLE_ROWS-1][1];
			lpb = (uint32)ggnfs_table[GGNFS_TABLE_ROWS-1][2];
			mfb = (uint32)ggnfs_table[GGNFS_TABLE_ROWS-1][3];
			lambda = ggnfs_table[GGNFS_TABLE_ROWS-1][4];
			siever = (uint32)ggnfs_table[GGNFS_TABLE_ROWS-1][5];
			if (job->rlim == 0) job->rlim = fblim;
			if (job->alim == 0) job->alim = fblim;
			if (job->lpbr == 0) job->lpbr = lpb;
			if (job->lpba == 0) job->lpba = lpb;
			...
			if (job->alambda == 0) job->alambda = lambda;
			if (fobj->nfs_obj.siever == 0) fobj->nfs_obj.siever = siever;
			job->qrange = (uint32)ggnfs_table[GGNFS_TABLE_ROWS-1][7];
		}
	}
