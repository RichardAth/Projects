	...
	mp_t *mpN, factor_list_t *factor_list)
{
	...
	for (i=0; i<THREADS; i++)
	{
		...
		init_poly_threaddata(t, obj, mpN, factor_list, (int)i, flags,
			1, 1001);
	...
	*threads_waiting = THREADS;
	...
	while (1)
	{
		...
		while (*threads_waiting > 0)
		{
			...
			this_range_time = (uint32)((double)difference->secs + (double)difference->usecs / 1000000);			
			free(difference);	
	...
	uint64 start, uint64 stop)
{
	...
	mallocCheck(nfs_args)
	t->logfilename = (char *)malloc(80 * sizeof(char));
	mallocCheck(t->logfilename)
	t->polyfilename = (char *)malloc(80 * sizeof(char));
	mallocCheck(t->polyfilename)
	t->fbfilename = (char *)malloc(80 * sizeof(char));
	mallocCheck(t->fbfilename)
	/* we assume, without checking, that the file names will fit! 
	Could we use snprintf_s instead of sprintf, or specify precision in %s? */
