...
void firstRoots_32k(static_conf_t *sconf, dynamic_conf_t *dconf)
{
	...
	int prime, amodp, bmodp, inv, x, bnum,j,numblocks;
	int s = poly->s;
	int bound_index = 0, k;
	...
	for (i=start_prime;i<sconf->sieve_small_fb_start;i++)
	{
		...
		update_data.firstroots1[i] = (int)root1;
		update_data.firstroots2[i] = (int)root2;
	...
	for (i=fb->med_B;i<fb->large_B;i++)
	{
		...
		root1 = (uint32)((uint64)inv * root1 % (int64)prime);
		root2 = (uint32)((uint64)inv * root2 % (int64)prime);
