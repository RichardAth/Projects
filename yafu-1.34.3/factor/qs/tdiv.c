						  ...
						  static_conf_t *conf)
{
	...
	else
	{
		...
		//store to file
		i = sprintf_s(buf, sizeof(buf), "R ");

		if (parity)
			i += sprintf_s(buf + i, sizeof(buf)-i, "-%x ", offset);
		else
			i += sprintf_s(buf + i, sizeof(buf) - i, "%x ", offset);
	
		i += sprintf_s(buf + i, sizeof(buf) - i, "%x ", poly_id);
	
		k = 0;
		while (k < num_factors)
			i += sprintf_s(buf + i, sizeof(buf) - i, "%x ", fb_offsets[k++]);

		if (large_prime[0] < large_prime[1])
			i += sprintf_s(buf + i, sizeof(buf) - i, "L %x %x\n", large_prime[0], large_prime[1]);
		else
