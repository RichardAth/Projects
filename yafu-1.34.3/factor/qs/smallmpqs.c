...
void smpqs_make_fb_mpqs(fb_list_sm_mpqs *fb, uint32 *modsqrt, mpz_t n)
{
	...
	uint8 logp;
	mpz_t tmpr;
	long long bits_n = mpz_sizeinbase(n,2);
	...
	while (j < fb->B)
	{
		r = (uint32)mpz_tdiv_ui(n, (fp_digit)spSOEprimes[i]); //r = (uint32)zShortMod(n,(fp_digit)spSOEprimes[i]);
		if (r == 0)
		{
			...
			b = (uint32)mpz_tdiv_ui(n, prime); //b = (uint32)zShortMod(n,(fp_digit)prime);
			k=0;
			while (1)
			{
				if (((k*k) % prime) == b)
...
void smallmpqs(fact_obj_t *fobj)
{
	...
	if (bits_n < 60)
	{
		...
		j = (uint32)sp_shanks_loop(n, fobj);	
		if (j > 1)
		{
			mpz_set_64(ztmp, j);
	...
	sm_get_params((int)bits_n, &j, &sm_sieve_params.large_mult, 
		&sm_sieve_params.num_blocks);
	...
	poly->poly_d = (uint32)mpz_get_ui(tmp); //.val[0];
						  ...
						  uint8 small_cutoff, uint32 start_prime, int numpoly, uint32 parity,uint8 closnuf,uint8 bits)
{
	...
	{
		smpqs_save_relation(partial,offset, (uint32)mpz_get_ui(Q),
			smooth_num+1, num_p, fboffset, numpoly, parity);
...
void smpqs_nextD(sm_mpqs_poly *poly, mpz_t n)
{
	...
	if (poly->side || poly->use_only_p)
	{
		...
		{
			poly->poly_d_idp++;
			if ((long long)poly->poly_d_idp >= (long long)szSOEp)
				smpqs_get_more_primes(poly);
			poly->poly_d = spSOEprimes[poly->poly_d_idp];
			r = (uint32)mpz_tdiv_ui(n,poly->poly_d); //zShortMod(n,*polyd);
...
void gmpModExp_1(mpz_t a, uint32 b, uint32 m, uint32 *u)
{
	...
	while (bb != 0)
	{
		if (bb & 0x1)
		{
			...
			n = (uint32)mpz_tdiv_ui(t, m); //n = zShortMod(&t,m);   //n*a mod m
		}
		bb >>= 1;   
		//compute successive squares of a
		mpz_mul(t,aa,aa); //zSqr(&aa,&t);
		ut = (uint32)mpz_tdiv_ui(t, m); //n = zShortMod(&t,m);
	...
	smpqs_sieve_fb *fbp, smpqs_sieve_fb *fbn, uint32 start_prime)
{
	...
	for (i=2;i<7;i++)
	{
		...
		tmp = q64 >> 32; 
		root1 = (int)(t2 - tmp * prime);

		t2 = (uint64)amodp * (uint64)root2;
		tmp = t2 + (uint64)fb->list->correction[i];
		q64 = tmp * (uint64)fb->list->small_inv[i];
		tmp = q64 >> 32; 
		root2 = (int)(t2 - tmp * prime);
			...
			mpz_t *factors,uint32 *num_factor)
{
	...
	*num_factor=0;
						...
						if (a==0)
						{
							...
							if (mul > 1) {
								uint32 ignore_me = spGCD(mul,
										(fp_digit)mpz_tdiv_ui(nn, mul)); //zShortMod(&nn, mul));
								if (ignore_me > 1) {
	...
	r = (uint32)mpz_tdiv_q_ui(tmp, n, mul); //r = (uint32)zShortDiv(n,mul,&tmp);
