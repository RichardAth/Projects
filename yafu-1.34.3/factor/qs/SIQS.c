...
void SIQS(fact_obj_t *fobj)
{
	...
	fobj->bits = (uint32)mpz_sizeinbase(fobj->qs_obj.gmp_n, 2);
	...
	for (i=0; i<THREADS; i++)
	{
		...
		mallocCheck(thread_data[i].dconf)
	...
	if (0) //(static_conf->digits_n < 1)
	{
		...
		mallocCheck(static_conf->in_mem_relations)
    ...
    *threads_waiting = THREADS;
	...
	{
		for (uint32 i=0; i<static_conf->total_poly_a + 1; i++)
	...
	mallocCheck(static_conf->curr_b)
	for (i = 0; i < 2; i++)
...
static int siqs_dynamic_init(dynamic_conf_t *dconf, static_conf_t *sconf)
{
	...
	mallocCheck(dconf->curr_poly->qlisort)
	...
	mallocCheck(dconf->relation_buf)
...
static int siqs_static_init(static_conf_t *sconf, int is_tiny)
{
	...
	while (1)
	{
		...
		mallocCheck(sconf->modsqrt_array)
		...
		sconf->bits = (uint32)mpz_sizeinbase(sconf->n, 2);
	...
	if (sconf->use_dlp)
	{
		...
		sconf->dlp_lower = bits64(sconf->max_fb2); 
		sconf->large_prime_max2 = (uint64)pow((double)sconf->large_prime_max,1.8);
		sconf->dlp_upper = bits64(sconf->large_prime_max2);
	}
...
static void free_filter_vars(static_conf_t *sconf)
{
	...
	for (i=0; (long long)i < (long long)sconf->num_cycles; i++)
	{
