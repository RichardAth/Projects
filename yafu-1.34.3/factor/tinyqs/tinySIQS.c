			...
			mpz_t *factors, uint32 *num_factor)
{
	...
	*num_factor=0;
					...
					if (bool_val)
					{
						...
						if (a==0)
						{
							...
							for (l=0;l<num_r;l++)
							{
								...
								//bool_val = (((aug_64[(k)][(l >> 6)]) & (1ULL << ((uint64)l & 63ULL)))) != 0;
								if (bool_val)
								{    // code below can't work because apoly and bpoly are never set up
	...
	r = (int)mpz_tdiv_q_ui(tmp, n, mul); //r = (uint32)zShortDiv(n,mul,&tmp);
