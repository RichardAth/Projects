...
/* basic check that malloc worked; if it failed then error msg & exit(-1)*/
...
//----------------------- LOCAL FUNCTIONS -------------------------------------//
static double best_linear_fit(double *x, double *y, int numpts, 
	double *slope, double *intercept);
static void update_INI(double mult, double exponent, double mult2, double exponent2, double xover);
static void make_job_file(char *sname, uint32 *startq, uint32 *qrange, char *inputstr, int inputnum, fact_obj_t *fobj);
...
void factor_tune(fact_obj_t *inobj)
{
	...
	char siqslist[9][200] = {
		"349594255864176572614071853194924838158088864370890996447417",       //C60
		"34053408309992030649212497354061832056920539397279047809781589871",  //C65
		"6470287906463336878241474855987746904297564226439499503918586590778209",
		"281396163585532137380297959872159569353696836686080935550459706878100362721",
		"33727095233132290409342213138708322681737322487170896778164145844669592994743377",
		...
		"1802716097522165018257858828415111497060066282677325501816640492782221110851604465066510547671104729"  //C100
	};
	char nfslist[6][200] = {
	"1877138824359859508015524119652506869600959721781289179190693027302028679377371001561",     //C80
	"427351849650748515507228344120452096326780093349980867041485502247153375067354165128307841",
	"48404068520546498995797968938385187958997290617596242601254422967869040251141325866025672337021",
	"1802716097522165018257858828415111497060066282677325501816640492782221110851604465066510547671104729",
	"466734409955806375058988820327650664396976790744285564594552020197119774272189758795312820988691316775181",
	"48178889479314834847826896738914354061668125063983964035428538278448985505047157633738779051249185304620494013"  // c15
	};
	z n;
	int i, tmpT;
	...
	//strcpy_s(siqslist[0], 200,"349594255864176572614071853194924838158088864370890996447417");
	//strcpy_s(siqslist[1], 200,"34053408309992030649212497354061832056920539397279047809781589871");
	//strcpy_s(siqslist[2], 200,"6470287906463336878241474855987746904297564226439499503918586590778209");
	//strcpy_s(siqslist[3], 200,"281396163585532137380297959872159569353696836686080935550459706878100362721");
	//strcpy_s(siqslist[4], 200,"33727095233132290409342213138708322681737322487170896778164145844669592994743377");
	//strcpy_s(siqslist[5], 200,"1877138824359859508015524119652506869600959721781289179190693027302028679377371001561");
	//strcpy_s(siqslist[6], 200,"427351849650748515507228344120452096326780093349980867041485502247153375067354165128307841");
	//strcpy_s(siqslist[7], 200,"48404068520546498995797968938385187958997290617596242601254422967869040251141325866025672337021");
	//strcpy_s(siqslist[8], 200,"1802716097522165018257858828415111497060066282677325501816640492782221110851604465066510547671104729");
	...
	/*strcpy_s(nfslist[0], 200,"1877138824359859508015524119652506869600959721781289179190693027302028679377371001561");
	strcpy_s(nfslist[1], 200,"427351849650748515507228344120452096326780093349980867041485502247153375067354165128307841");
	strcpy_s(nfslist[2], 200,"48404068520546498995797968938385187958997290617596242601254422967869040251141325866025672337021");
	strcpy_s(nfslist[3], 200,"1802716097522165018257858828415111497060066282677325501816640492782221110851604465066510547671104729");
	strcpy_s(nfslist[4], 200,"466734409955806375058988820327650664396976790744285564594552020197119774272189758795312820988691316775181");
	strcpy_s(nfslist[5], 200,"48178889479314834847826896738914354061668125063983964035428538278448985505047157633738779051249185304620494013");*/
	...
	for (i=0; i<NUM_GNFS_PTS; i++)
	{
		...
		sprintf_s(syscmd, sizeof(syscmd), "%s -b tune.job -k -c 0 -F", sievername);
		...
		//start the test
		sprintf_s(syscmd, sizeof(syscmd), "%s -a tune.job -f %u -c %u -o tunerels.out",
...
static void make_job_file(char *sname, uint32 *startq, uint32 *qrange, char *inputstr, int inputnum, fact_obj_t *fobj)
{
	...
	*qrange = 5000;
	...
	case 4:
		...
		fprintf(out, "alambda: 2.5\n");
		siever = 12;
		*startq = 2500000; // changed from 1250000
	...
	{
	case 11:
		sprintf_s(sname, 1024, "%sgnfs-lasieve4I11e", fobj->nfs_obj.ggnfs_dir);
		break;
	case 12:
		sprintf_s(sname, 1024, "%sgnfs-lasieve4I12e", fobj->nfs_obj.ggnfs_dir);
		break;
	case 13:
		sprintf_s(sname, 1024, "%sgnfs-lasieve4I13e", fobj->nfs_obj.ggnfs_dir);
		break;
	case 14:
		sprintf_s(sname, 1024, "%sgnfs-lasieve4I14e", fobj->nfs_obj.ggnfs_dir);
		break;
	case 15:
		sprintf_s(sname, 1024, "%sgnfs-lasieve4I15e", fobj->nfs_obj.ggnfs_dir);
		break;
	default:
		printf("siever value %d invalid should be in range 11 to 15 \n", 
			siever);
		exit(-1);
	}

#if defined(WIN32)
	sprintf_s(sname, 1024, "%s.exe", sname);
#endif
...
static void update_INI(double mult, double exponent, double mult2, double exponent2, double xover)
{
	...
	while (fgets(str,GSTR_MAXSIZE,in) != NULL)
	{
		...
		else if (strcmp(key,"tune_info") == 0)
		{
			...
			if ((strcmp(cpustr,CPU_ID_STR) == 0) && (strcmp(osstr, "WIN64") == 0))
			{
				...
				found_entry = 1;
				sprintf_s(newline, sizeof(newline), 
					"tune_info=%s,WIN64,%lg,%lg,%lg,%lg,%lg,%lg\n",
	...
	if (!found_entry)
	{
		...
		sprintf_s(newline, sizeof(newline), "tune_info=%s,WIN64,%lg,%lg,%lg,%lg,%lg,%lg\n",
			CPU_ID_STR,mult,exponent,mult2,exponent2,xover,MEAS_CPU_FREQUENCY);
		fputs(newline, out);
#elif defined(WIN32)
	...
	double *slope, double *intercept)
{
	...
	for (i = 0; i < numpts; i++) {
		if (y[i] <= 0.0) {// ** if y is -ve log is not a real number!
			printf("tune failure at line %d \n", __LINE__);
			printf("i =%d, y[i] =%g numpts = %d \n", i, y[i], numpts);
			DumpStackTrace2();
			exit(1);
		}
		yy[i] = log(y[i]);
	...
	avgX = 0;
	for (i=0; i<numpts; i++)
		if (fpclassify(x[i]) == FP_NORMAL || fpclassify(x[i]) == FP_SUBNORMAL)
			avgX += x[i];
	avgX = avgX / (double)numpts;

	avgY = 0;
	for (i=0; i<numpts; i++)
		if (fpclassify(yy[i]) == FP_NORMAL || fpclassify(yy[i]) == FP_SUBNORMAL)
		avgY += yy[i];
