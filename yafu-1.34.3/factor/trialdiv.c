...
void zTrial(fact_obj_t *fobj)
{
	...
	{
		q = (fp_digit)PRIMES[k];
		r = (uint32)mpz_tdiv_ui(fobj->div_obj.gmp_n, q);
...
void factor_perfect_power(fact_obj_t *fobj, mpz_t b)
{
	...
	uint32 bits = (uint32)mpz_sizeinbase(b,2);
	uint32 i;
	FILE *flog;
	mpz_t base, ans;
		...
		if (mpz_cmp(ans, b) == 0)
		{
			...
			else
			{
				...
				for (j=0; j< fobj_refactor->num_factors; j++)
				{
					...
					for (int k=0; k < fobj_refactor->fobj_factors[j].count; k++)
					{
...
void zFermat(uint64 limit, uint32 mult, fact_obj_t *fobj)
{
	...
	// b^2 can be a square
	m = (uint32)mpz_mod_ui(tmp, a, M);
	mmn = (uint32)mpz_mod_ui(tmp, b2, M);
	...
	m = (uint32)mpz_mod_ui(tmp, a, M1);
	mmn = (uint32)mpz_mod_ui(tmp, b2, M1);
	...
	m = (uint32)mpz_mod_ui(tmp, a, M2);
	mmn = (uint32)mpz_mod_ui(tmp, b2, M2);
	for (uint32 i = 0; i < M2; ++i)
	{
...
uint64 spfermat(uint64 n, uint64 limit)
{
	...
	if (sptestsqr(b2))
		return a + (uint64)sqrt((double)b2);
	else
		return 1;
