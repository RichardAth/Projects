	...
	uint64 start_count, uint64 *primes)
{
	...
	range = (uint32)(sdata->numlinebytes / THREADS);
	range -= (range % 8);
	lastid = 0;
	...
	for (j = 0; j < THREADS; j++)
	{
		...
		{	
			...
			{
				t->linecount = compute_8_bytes(sdata, (uint32)t->linecount, 
					t->ddata.primes, i, &pchar);		
			}
		}
		else
		{
	...
	if (THREADS > 1)
	{
		...
		for (j = 0; j < THREADS; j++)
		{
			...
			pcount += (uint32)t->linecount;
			free(t->ddata.primes);
		}
	}
	else
		pcount = (uint32)thread_data[0].linecount;
	...
	if (lastid != sdata->numlinebytes)
	{
		...
		{
			pcount = compute_8_bytes(sdata, (uint32)pcount, primes, i, NULL);		
		}
	}
