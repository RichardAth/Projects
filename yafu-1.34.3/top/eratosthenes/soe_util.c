...
void get_numclasses(uint64 highlimit, uint64 lowlimit, soe_staticdata_t *sdata)
{
	...
	sdata->numclasses = (uint32)numclasses;
	sdata->prodN = prodN;
	...
	soe_staticdata_t *sdata, mpz_t offset)
{
	...
	if (offset == NULL)
	{
		...
		sdata->pbound = (uint64)(sqrt((double)(highlimit)));
...
uint64 init_sieve(soe_staticdata_t *sdata)
{
	...
	if (sdata->pboundi > BUCKETSTARTI)
	{
		...
		sdata->num_bucket_primes = 
			(uint32)(sdata->pboundi - sdata->bucket_start_id);		
	}
	else
	{
		sdata->num_bucket_primes = 0;
		sdata->bucket_start_id = (uint32)sdata->pboundi;
	}

	//any prime larger than this will only hit the interval once (in residue space)
	sdata->large_bucket_start_prime = (uint32)(sdata->blocks * FLAGSIZE);
	...
	sdata->inplace_start_id = (uint32)sdata->pboundi;
	...
	mallocCheck(sdata->lower_mod_prime)
	...
	else
	{
		...
		for (uint32 i=0; i<sdata->numclasses; i++)
		{
			sdata->lines[i] = (uint8 *)malloc(numlinebytes * sizeof(uint8));
			mallocCheck(sdata->lines[i])
...
void set_bucket_depth(soe_staticdata_t *sdata)
{
	...
	if (sdata->num_bucket_primes > 0)
	{
		...
		for (i = sdata->bucket_start_id; (uint32)i < sdata->bucket_start_id + sdata->num_bucket_primes; i++)
		{
		...
		sdata->bucket_alloc = (uint32)MAX(hits_per_bucket, 50000);

		//now count primes that only hit the interval once
		num_hits = 0;
		for (; (uint32)i < sdata->bucket_start_id + sdata->num_bucket_primes; i++)
			num_hits++;
		...
		if (num_hits > 0)
			sdata->large_bucket_alloc =(uint32) MAX(hits_per_bucket, 50000);
		else
...
uint64 alloc_threaddata(soe_staticdata_t *sdata, thread_soedata_t *thread_data)
{
	...
	for (i=0; i<THREADS; i++)
	{
		...
		mallocCheck(thread->ddata.pbounds)
		...
		j = (uint32)MIN(sdata->pboundi, BUCKETSTARTI);
		thread->ddata.offsets = (uint32 *)malloc(j * sizeof(uint32));
		mallocCheck(thread->ddata.offsets)
		...
		{			
			...
			mallocCheck(thread->ddata.sieve_buckets)
			...
			if (large_bucket_alloc > 0)
			{
				...
				mallocCheck(thread->ddata.large_sieve_buckets)
			...
			thread->ddata.bucket_hits = (uint32 *)malloc(
				sdata->blocks * sizeof(uint32));
			mallocCheck(thread->ddata.bucket_hits);
			allocated_bytes += sdata->blocks * sizeof(uint32);
			...
			if (large_bucket_alloc > 0)
			{
				thread->ddata.large_bucket_hits = (uint32 *)malloc(
					sdata->blocks * sizeof(uint32));
				mallocCheck(thread->ddata.large_bucket_hits);
				allocated_bytes += sdata->blocks * sizeof(uint32);
			...
			for (j = 0; j < sdata->blocks; j++)
			{
				...
				mallocCheck(thread->ddata.sieve_buckets[j]);
...
void build_table()
{
	...
	exit(0);  /***** this stops the program. Why??? ********/
