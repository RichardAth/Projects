...
/* warning 4996 is for functions that Microsoft consider 'unsafe' 
The pragma is less drastic than CRT_SECURE_NO_DEPRECATE */
#ifdef _MSC_VER
#include <intrin.h>
...
/* copy src to dest */
void toStr(const char *src, str_t *dest)
{
	if ((int)strlen(src) > dest->alloc)
	{
		sGrow(dest, (int)strlen(src) + 10);
		dest->alloc = (int)strlen(src) + 10;
	}
	memcpy(dest->s, src, strlen(src) * sizeof(char));
	dest->s[strlen(src)] = '\0';
	dest->nchars = (int)strlen(src) + 1;
...
void sAppend(const char *src, str_t *dest)
{
	...
	{
		sGrow(dest,(int)strlen(src) + dest->nchars + 10);
		dest->alloc = (int)strlen(src) + dest->nchars + 10;
	}

	memcpy(dest->s + dest->nchars - 1,src,strlen(src) * sizeof(char));
	dest->nchars += (int)strlen(src);	//already has a null char accounted for
	dest->s[dest->nchars - 1] = '\0';

	return;
}

void sCopy(str_t *dest, const str_t *src)
{
...
/* return time as an ascii string */
char * time_from_secs(char *str, unsigned long long time)
{
	...
	if( time > 3600*24 )
	{
		d = time / (3600*24);
		time %= (3600*24);
		sprintf(str, "%llu day%s ", d, d>1?"s":"");
	}
	if( time > 3600 )
	{
		d = time / 3600;
		time %= 3600;
		sprintf(str, "%s%lluh ", str, d);
	}
	if( time > 60 )
	{
		d = time / 60;
		time %= 60;
		sprintf(str, "%s%llum ", str, d);
	}
	sprintf(str, "%s%llus", str, time);
	return str;
}
...
static void gordon(int bits, z *p)
{
	/*
  ...
  4.54 Note (implementing Gordon’s algorithm)
	...
	*/
	...
	//2. Select an integer i0. Find the first prime in the sequence 2it + 1, for i = i0; 
	// i0 + 1; i0 + 2; : : : (see Note 4.54). Denote this prime by r = 2it+ 1.
	i=1;
	zShiftLeft(&r, &t, 1);     // r = 2*t
	zShortAdd(&r, 1, &r);      // r = 2*t +1
	while (!isPrime(&r))
	{
		i++;
		zShiftLeft(&r, &t, 1);   // r = 2*t
		zShortMul(&r, i, &r);    // r = 2*t*i 
		zShortAdd(&r, 1, &r);    // r = 2*t*i +1
	}

	//3. Compute p0 = 2(sr-2 mod r)s - 1.
	zMul(&s, &r, &tmp);          // tmp  = s*r
	zShortSub(&tmp, 2, &p0);       // tmp  = s*r -2
	zDiv(&p0, &r, &tmp, &tmp2);  // tmp2 = (s*r-2)%r
	zMul(&tmp2, &s, &p0);        // p0   = ((s*r-2)%r)*s
	zShiftLeft(&p0, &p0, 1);     // p0   = ((s*r-2)%r)*s*2
	zShortSub(&p0, 1, &p0);      // p0   = ((sr*2-2)%r)*s*2-1
	...
	j=1;
	zMul(&r, &s, &tmp);             // tmp = r*s
	zShiftLeft(&tmp, &tmp, 1);      // tmp = 2*r*s
	zAdd(&p0, &tmp, p);             // p = p0 + 2*r*s
	while (!isPrime(p))
	{
		j++;
		zMul(&r, &s, &tmp);           // tmp = r*s
		zShiftLeft(&tmp, &tmp, 1);    // tmp = 2*r*s
		zShortMul(&tmp, j, &tmp);     // tmp = 2*r*s*j
		zAdd(&p0, &tmp, p);           // p = p0 + 2*r*s*j
	}
...
void build_RSA(int bits, mpz_t in)
{
	...
	while (zBits(&n) != bits)
	{
		gordon(bits/2, &p);   // random strong prime size bits/2
		gordon(bits/2, &q);
		zMul(&p, &q, &n);
...
/* attempt to automatically detect the size of the L2 cache; 
this helps tune the choice of parameters or algorithms used in the 
sieve-based methods. It should guess right for most PCs and Macs 
when using gcc.

Otherwise, you have the source so just fill in the correct number. */
...
/* binary chop search of input for value q. if q is not in input
return index of largest value smaller than q. */
int bin_search_uint32(int idp, int idm, uint32 q, const uint32 *input)
{
	int next = (idp + idm) / 2;
