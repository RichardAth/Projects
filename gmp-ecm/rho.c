...
dickmanrho (double alpha)
{
  ...
  {
    long long a = (long long)floor (alpha * invh);
...
static double
dickmanmu_sum (const unsigned long long B1, const unsigned long long B2, 
	       const double x)
{
  double s = 0.;
  const double logB1 = 1. / log((double)B1);
  const double logx = log(x); 
  unsigned long long p;

  for (p = B1 + 1; p <= B2; p++)
    if (isprime(p))
      s += dickmanlocal ((logx - log((double)p)) * logB1, x / p) / p;
...
dickmanmu (double alpha, double beta, double x)
{
  double a, b, sum;
  int ai, bi, i;
  ai = (int)ceil ((alpha - beta) * invh);
  if (ai > tablemax * invh)
    ai = tablemax * invh;
  a = (double) ai * h;
  bi = (int)floor ((alpha - 1.) * invh);
  if (bi > tablemax * invh)
    bi = tablemax * invh;
  b = (double) bi * h;
  sum = 0.;
  ...
  if (B2 > B1)
    {
      if (B1 < sumthresh)
	{
	  stage2 += dickmanmu_sum ((unsigned long long)B1, 
		  (unsigned long long)MIN(B2, sumthresh), effN);
