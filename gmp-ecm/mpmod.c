...
isbase2 (const mpz_t n, const double threshold)
{
  ...
  mpz_init (u);
  mpz_init (w);
  lo = (unsigned int)mpz_sizeinbase (n, 2) - 1; /* 2^lo <= n < 2^(lo+1) */  
  mpz_set_ui (u, 1UL);
  ...
  k = (unsigned int)mpz_sizeinbase (w, 2) - 1;
...
{
  unsigned __int64 absbits = abs (modulus->bits);
...
mpmod_init_MPZ (mpmod_t modulus, const mpz_t N)
{
  ...
  modulus->bits = (unsigned int)n * GMP_NUMB_BITS; /* Number of bits,
...
mpmod_init_BASE2 (mpmod_t modulus, const int base2, const mpz_t N)
{
  ...
  Nbits = (unsigned int)mpz_size (N) * GMP_NUMB_BITS; /* Number of bits, rounded
                                           up to full limb */
...
mpmod_init_REDC (mpmod_t modulus, const mpz_t N)
{
  ...
  n = mpz_size (N);
  modulus->repr = ECM_MOD_REDC;
  Nbits = (unsigned int)n * GMP_NUMB_BITS; /* Number of bits, rounded
                                up to full limb */
  modulus->bits = Nbits;
...
{
  const unsigned __int64 Nbits = abs(modulus->bits);
  const unsigned __int64 n = (unsigned int)mpz_size (modulus->orig_modulus);
...
mpres_pow (mpres_t R, const mpres_t BASE, const mpz_t EXP, mpmod_t modulus)
{
  ...
  if (modulus->repr == ECM_MOD_MPZ)
           ...
           modulus->repr == ECM_MOD_REDC)
    {
      ...
      while ((1LL << (k-1)) + expnbits / (k + 1) > (1LL << k) + expnbits / (k + 2))
        k ++;
...
/* R <- BASE^EXP mod modulus */ 
void 
mpres_ui_pow (mpres_t R, const unsigned __int64 BASE, const mpres_t EXP, 
              mpmod_t modulus)
{
  if (modulus->repr == ECM_MOD_MPZ)
    {
  ...
  if (UNLIKELY(modulus->repr == ECM_MOD_BASE2 && modulus->Fermat >= 32768))
    {
      ...
      PTR(R)[n] = mpn_mul_fft (PTR(R), n, s1p, ABS(s1s), s2p, ABS(s2s), (int)k);
      n ++;
...
/* R <- S * n mod modulus */
void 
mpres_mul_ui (mpres_t R, const mpres_t S, const unsigned __int64 n, 
              mpmod_t modulus)
{
  ASSERT_NORMALIZED (S);
...
/* R <- S * 2^k mod modulus */
void 
mpres_mul_2exp (mpres_t R, const mpres_t S, const unsigned __int64 k, 
              mpmod_t modulus)
{
  ASSERT_NORMALIZED (S);
...
void
mpres_add_ui (mpres_t R, const mpres_t S, const unsigned __int64 n, 
              mpmod_t modulus)
{
  ASSERT_NORMALIZED (S);
...
*/
void
mpres_sub_ui (mpres_t R, const mpres_t S, const unsigned __int64 n, 
              mpmod_t modulus)
{
  ASSERT_NORMALIZED (S);
  if (modulus->repr == ECM_MOD_MPZ || modulus->repr == ECM_MOD_BASE2)
...
*/
void
mpres_ui_sub (mpres_t R, const unsigned __int64 n ,const mpres_t S, 
              mpmod_t modulus)
{
  ASSERT_NORMALIZED (S);
  if (modulus->repr == ECM_MOD_MPZ || modulus->repr == ECM_MOD_BASE2)
 ...
 */
void 
mpres_set_ui (mpres_t R, const unsigned __int64 n, mpmod_t modulus)
{
  if (modulus->repr == ECM_MOD_MPZ || modulus->repr == ECM_MOD_BASE2)
    {
      mpz_set_ui (R, n);
...
void 
mpres_set_si (mpres_t R, const __int64 n, mpmod_t modulus)
{
  if (modulus->repr == ECM_MOD_MPZ || modulus->repr == ECM_MOD_BASE2)
    {
      mpz_set_si (R, n);
...
/* Multiplies S by n and possibly divides by some constant. 
   ...
   the modulus size. */
void 
mpres_muldivbysomething_si (mpres_t R, const mpres_t S, const __int64 n, 
			    mpmod_t modulus)
{
  ASSERT_NORMALIZED (S);
  if (modulus->repr == ECM_MOD_MODMULN && 
...
mpresn_mul (mpres_t R, const mpres_t S1, const mpres_t S2, mpmod_t modulus)
{
  ...
  SIZ(R) = SIZ(S1) == SIZ(S2) ? (int)n : -(int)n;
}
