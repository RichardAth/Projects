	      ...
	      int (*stop_asap)(void), char *chkfilename)
{
	...
	for (r = 2; r <= B1; r *= 2)
		...
		ell_point_set(P, Q, E, n);
#if DEBUG_EC_W >= 2
		printf("P%ld:=", (__int64)r); ell_point_print(P, E, n); printf(";\n");
...
#if DEBUG_EC_W >= 2
		    printf("R%ld:=", (__int64)r); ell_point_print(Q, E, n);
		    printf(";\nQ:=EcmMult(%ld, Q, E, N);\n", (__int64)p);
...
void
print_expcurves (double B1, const mpz_t B2, unsigned long long dF, unsigned long long k, 
                 int S, int param)
{
  double prob;
  int i, j;
...
void
print_exptime (double B1, const mpz_t B2, unsigned long long dF, unsigned long long k, 
...
set_stage_2_params (mpz_t B2, mpz_t B2_parm, mpz_t B2min, mpz_t B2min_parm, 
                    ...
                    unsigned long long *k, const int S, int use_ntt, int *po2,
                    unsigned long long *dF, char *TreeFilename, double maxmem, 
...
ecm (mpz_t f, mpz_t x, mpz_t y, int *param, mpz_t sigma, mpz_t n, mpz_t go, 
     ...
     unsigned long long k, const int S, int verbose, int repr, int nobase2step2, 
     ...
     *TreeFilename, double maxmem, double stage1time, gmp_randstate_t rng, int
     ...
     ATTRIBUTE_UNUSED double gw_k, ATTRIBUTE_UNUSED unsigned __int64 gw_b,
     ATTRIBUTE_UNUSED unsigned __int64 gw_n, ATTRIBUTE_UNUSED signed __int64 gw_c)
{
  ...
  if (IS_BATCH_MODE(*param) && ECM_IS_DEFAULT_B1_DONE(*B1done) &&
      (B1 != *batch_last_B1_used || mpz_cmp_ui (batch_s, 1) <= 0))
    {
      *batch_last_B1_used = B1;
      ...
      compute_s (batch_s, (ecm_uint)B1, NULL);
  ...
  if (stage1time > 0.)
    {
      const long long st2 = elltime (st, cputime ());
      const __int64 s1t = (__int64) (stage1time * 1000.);
