...
get_random_ul (void)
{
  ...
  if (CryptAcquireContext (&Prov, NULL, NULL, PROV_RSA_FULL,
    ...
    {
      int r;
      unsigned __int64 rnd;
    
      r = CryptGenRandom (Prov, sizeof (unsigned __int64), (void *) &rnd);
...
get_random_ul (void)
{
  ...
  if (rndfd != NULL)
    {
      ...
      res = fread (&t, sizeof (unsigned __int64), 1, rndfd);
      fclose (rndfd);
      if (res == 1)
        return t;
    }
  ...
  return (unsigned __int64) time (NULL) * 1431655751UL +
         (unsigned __int64) getpid () * 2147483629UL;
}
#endif
