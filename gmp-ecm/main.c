...
#define MNAMESIZE  64
  ...
  while ((cnt > 0 || feof (infile) == 0) && !exit_asap_value)
          ...
          if (read_s_from_file (params->batch_s, loadfile_s, B1))
            {
              ...
              fprintf (stdout, "Reading batch product (of %"PRIu64" bits) of "
                               "primes up to B1=%1.0f from %s took %lldms\n", 
      ...
      if (resumefile == NULL)
	{
      ...
      if (kbnc_z (&gw_k, &gw_b, &gw_n, &gw_c, n.n))
        {
          ...
          params->gw_c = gw_c;
          if (verbose > OUTPUT_NORMAL)
            printf ("Found number: %.0f*%lu^%lu + %lld\n",
                    gw_k, gw_b, gw_n, gw_c);
        }
      ...
      if (savefile_s != NULL)
        {
          long long ret = write_s_in_file (savefile_s, params->batch_s);
          if (verbose >= OUTPUT_VERBOSE && ret > 0)
              printf ("Saved batch product (of %llu bytes) in %s\n", ret, 
                      savefile_s);
        }
 ...
 free_all:
  ...
  if (verbose > 1)
    {
      long long peak = PeakMemusage ();
      if (peak != -1)
        printf ("Peak memory usage: %lldMB\n", peak >> 10);
    }
