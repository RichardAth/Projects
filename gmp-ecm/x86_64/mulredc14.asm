...
`	movq	%rsi, %XP		# store x in XP
	...
	subq	$120, %rsp	# subtract size of local vars
...
`#' Pass for j = 13. Don't fetch new data from y[j+1].
	...
	mulq    %U		# m[j]*u
	addq	%rax, %T0	# Add low word to T0
	movq	%T0, 96(%TP)	# Store T0 in tmp[j-1]
	adcq	%rdx, %T1	# Add high word with carry to T1
	movq	%T1, 104(%TP)	# Store T1 in tmp[j]
	setc	%CYb		# %CY <= 1
	movq	%CY, 112(%TP)	# Store CY in tmp[j+1]
