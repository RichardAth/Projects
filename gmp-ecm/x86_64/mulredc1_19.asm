# mp_limb_t mulredc1_19(mp_limb_t * z, const mp_limb_t x, const mp_limb_t * y,
...
# Pass for j = 18. Don't fetch new data from y[j+1].
	...
	mulq    U		# m[j]*u
	addq	%rax, T0	# Add low word to T0
	movq	T0, 136(ZP)	# Store T0 in z[j-1]
	adcq	%rdx, T1	# Add high word with carry to T1
	movq	T1, 144(ZP)	# Store T1 in tmp[j]
	setc	CYb		# %CY <= 1
