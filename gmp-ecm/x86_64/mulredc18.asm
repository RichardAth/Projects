# mp_limb_t mulredc18(mp_limb_t * z, const mp_limb_t * x, const mp_limb_t * y,
...
`	movq	%rsi, %XP		# store x in XP
	...
	subq	$152, %rsp	# subtract size of local vars
...
`#' Pass for j = 17. Don't fetch new data from y[j+1].
	...
	mulq    %U		# m[j]*u
	addq	%rax, %T0	# Add low word to T0
	movq	%T0, 128(%TP)	# Store T0 in tmp[j-1]
	adcq	%rdx, %T1	# Add high word with carry to T1
	movq	%T1, 136(%TP)	# Store T1 in tmp[j]
	setc	%CYb		# %CY <= 1
	movq	%CY, 144(%TP)	# Store CY in tmp[j+1]
