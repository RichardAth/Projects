...
static void
print_elapsed_time (int verbosity, long long cpu_start, 
		    ATTRIBUTE_UNUSED long long real_start)
{
#ifdef _OPENMP
  if (real_start != 0L)
    {
      outputf (verbosity, " took %llums (%llums real)\n", 
...
static size_t
ntt_coeff_mem (const unsigned long long lmax, const mpz_t modulus, const int twice)
{
  mpz_t t;
  size_t n;
...
size_t
pm1fs2_memory_use (const unsigned long long lmax, const mpz_t modulus, 
		   const int use_ntt)
{
  if (use_ntt)
    {
  ...
  else
    {
      ...
      */
      ...
      n *= 5 * lmax + lmax / 4 + list_mul_mem ((unsigned int)lmax / 2);
  ...
  if (use_ntt)
    {
      ...
      lmax = 1ULL << ceil_log2 (memory / n / 3);
      return lmax;
    }
  else
    {
...
size_t 
pp1fs2_memory_use (const unsigned long long lmax, const mpz_t modulus, 
	       ...
	       const int twopass)
{
  ...
  if (use_ntt)
	...
	n = memory / (3 * n);
      return 1ULL << (ceil_log2 (n / 2)); /* Rounded down to power of 2 */
    }
  else
    {
...
static int
test_P (const mpz_t B2min, const mpz_t B2, mpz_t m_1, const unsigned long long P, 
	const unsigned long long nr, mpz_t effB2min, mpz_t effB2)
{
  mpz_t m;
...
static void
factor_phiP (int *exponents, const unsigned long long phiP)
{
    const int nrprimes = sizeof (phiPfactors) / sizeof (unsigned long);
    unsigned long long cofactor = phiP;
    int i;
...
absdiff_ul (unsigned long long a, unsigned long long b)
{
    return (a > b) ? a - b : b - a;
}
...
static unsigned long long
choose_s_1 (const unsigned long long phiP, const unsigned long long min_s2,
	    const unsigned long long l, const int use_ntt)
{
  const int nrprimes = sizeof (phiPfactors) / sizeof (unsigned long);
...
static unsigned long long
est_cost (const unsigned long long s_2, const unsigned long long l, const int use_ntt,
          const int method)
{
  if (method == ECM_PM1)
    {
...
long long
choose_P (const mpz_t B2min, const mpz_t B2, const unsigned long long lmax,
	  const unsigned long long min_s2, faststage2_param_t *finalparams, 
	  ...
	  const int method)
{
  ...
  */
  ...
  /* The best parameters found so far, P == 0 means that no suitable P
     has been found yet: */
  unsigned long long P = 0, s_1 = 0, s_2 = 0, l = 0, cost = 0;
  unsigned int i;
  ...
  for (i = 0 ; i < Pvalues_len; i++)
    {
      unsigned long long tryP, tryphiP, trys_1, trys_2, tryl, trycost;
...
static void
list_output_poly (listz_t l, unsigned long long len, int monic, int symmetric,
...
static void
list_sqr_reciprocal (listz_t R, listz_t S, const unsigned long long l, 
		     mpz_t modulus, listz_t tmp, 
		     ATTRIBUTE_UNUSED const unsigned long long tmplen)
{
  unsigned long long i;
...
static void
list_mul_reciprocal (listz_t R, listz_t S1, unsigned long long l1, 
		     listz_t S2, unsigned long long l2,
		     mpz_t modulus, listz_t tmp, 
		     ATTRIBUTE_UNUSED const unsigned long long tmplen)
{
  unsigned long long i;
  const unsigned long long lmax = MAX(l1, l2);
...
static void
V (mpres_t R, const mpres_t S, const long long k, mpmod_t modulus)
{
  mpres_t V0, Vi, Vi1;
  unsigned long long j, uk;
  int po2;
...
static void
U (mpres_t R, mpres_t R1, const mpres_t S, const long long k, mpmod_t modulus)
{
...
static void
scale_by_chebyshev (listz_t R, const listz_t F, const unsigned long long len,
...
list_scale_V (listz_t R, const listz_t F, const mpres_t Q, 
              const unsigned long long deg, mpmod_t modulus, listz_t tmp, 
              const unsigned long long tmplen, 
	      mpzspv_t dct, const mpzspm_t ntt_context)
{
  mpres_t Vt;
  unsigned long long i;
  const listz_t G = tmp, H = tmp + 2 * deg + 1, newtmp = tmp + 4 * deg + 2;
  const unsigned long long newtmplen = tmplen - 4 * deg - 2;
#ifdef WANT_ASSERT
  mpz_t leading;
#endif
...
{
  unsigned long long c, deg, i, nr;
  set_long_t *set = sets->sets;
  unsigned long long mem, maxmem = 0;
...
poly_from_sets_V (listz_t F, const mpres_t Q, sets_long_t *sets, 
		  listz_t tmp, const unsigned long long tmplen, mpmod_t modulus,
		  mpzspv_t dct, const mpzspm_t ntt_context)
{
  unsigned long long c, deg, i, nr;
  set_long_t *set = sets->sets;
	     ...
	     const faststage2_param_t *params, mpmod_t modulus)
{
  ...
  tmp = init_list2 ((unsigned int)tmplen, (unsigned int) abs (modulus->bits));
  F_ntt = mpzspv_init (1ULL << ceil_log2 (params->s_1 / 2 + 1), F_ntt_context);
...
pm1_sequence_g (listz_t g_mpz, mpzspv_t g_ntt, const mpres_t b_1, 
                const unsigned long long P, const long long M_param, 
		const unsigned long long l_param, const mpz_t m_1, const long long k_2, 
		mpmod_t modulus_param, const mpzspm_t ntt_context)
{
#pragma warning( disable : 4101)  // suppress spurious warning about unused variables
...
#pragma warning( default : 4101)
  ...
  __int64 timestart, realstart;
  long long M = M_param;
  unsigned long long l = l_param, offset = 0UL;
...
pm1_sequence_h (listz_t h, mpzspv_t h_ntt, mpz_t *f, const mpres_t r, 
		const unsigned long long d, mpmod_t modulus_parm, 
		const mpzspm_t ntt_context)
{
...
#pragma omp parallel if (d > 100)
#endif
  {
    ...
    unsigned long long offset = 0ULL, len = d;
    mpmod_t modulus;
	...
	outputf (OUTPUT_VERBOSE, " using %d threads", nr_chunks);
      ...
      chunklen = (len - 1UL) / (unsigned long long) nr_chunks + 1UL;
              ...
              const faststage2_param_t *params)
{
  ...
  *S_2 = malloc (set_sizeof(params->s_2));
  ...
  facS_2 = malloc (facS_2_size+8);  // +8 is a kludge
  if (facS_2 == NULL)
    {
      free (*S_1);
      free (*S_2);
...
static void
ntt_gcd (mpz_t f, mpz_t *product, mpzspv_t ntt, const unsigned long long ntt_offset,
	 const listz_t add, const unsigned long long len_param, 
...
#pragma warning( disable : 4101)  // suppress spurious warning about unused variables
  ...
  mpmod_t modulus;
#pragma warning( default : 4101)
  const unsigned int Rlen = MPZSPV_NORMALISE_STRIDE;
  __int64 timestart, realstart;
  unsigned long long len = len_param, thread_offset = 0;
...
#pragma omp master
    ...
    R = init_list2 (Rlen, (unsigned int)(mpz_size (modulus->orig_modulus) + 2) * 
                           GMP_NUMB_BITS);
    ...
    for (i = 0; i < len; i += Rlen)
      {
	const unsigned long long blocklen = MIN(len - i, Rlen);
...
pm1fs2 (mpz_t f, const mpres_t X, mpmod_t modulus, 
	const faststage2_param_t *params)
{
  unsigned long long phiP, nr;
  unsigned long long i, l, lenF, lenG, lenR, tmplen;
  ...
  __int64 timetotalstart, realtotalstart, timestart;
  ...
  F = init_list2 ((unsigned int)lenF, (unsigned int) abs (modulus->bits));
  ...
  lenG = params->l;
  g = init_list2 ((unsigned int)lenG, (unsigned int) abs (modulus->bits));
  lenR = nr;
  R = init_list2 ((unsigned int)lenR, (unsigned int) abs (modulus->bits));
  tmplen = 3UL * params->l + list_mul_mem ((unsigned int)params->l / 2);
  outputf (OUTPUT_DEVVERBOSE, "tmplen = %lu\n", tmplen);
  if (TMulGen_space ((unsigned int)params->l - 1, (unsigned int)params->s_1, 
	  (unsigned int)lenR) + 12 > tmplen)     {
      tmplen = TMulGen_space ((unsigned int)params->l - 1, (unsigned int)params->s_1 - 1, 
		  (unsigned int)lenR) + 12;
  ...
  for (l = 0; l < params->s_2; l++)
    {
      const unsigned long long M = params->l - 1LL - params->s_1 / 2L;
	 ...
	 x^(s_1+nr-1) = x^(len-1) */
      if (TMulGen (R, (unsigned int)nr - 1, h, (unsigned int)params->s_1, g, 
		  (unsigned int)params->l - 1, tmp, modulus->orig_modulus) < 0)
	{
      ...
      list_mod (R, R, (unsigned int)nr, modulus->orig_modulus);
	...
	const faststage2_param_t *params)
{
  ...
  int youpi = ECM_NO_FACTOR_FOUND;
  __int64 timetotalstart, realtotalstart, timestart, realstart;
				     ...
				     the leading 1 monomial for each factor */
  {
    ...
    unsigned long long mem = mem_poly_from_sets_V (S_1);
    if (mem > lenF)
      lenF = mem;
  }
  ...
  for (l = 0; l < params->s_2; l++)
    {
      const unsigned long long M = params->l - 1L - params->s_1 / 2L;

      outputf (OUTPUT_VERBOSE, "Multi-point evaluation %llu of %llu:\n", 
               l + 1, params->s_2);
...
gfp_ext_mul (mpres_t r_0, mpres_t r_1, const mpres_t a_0, const mpres_t a_1,
	     ...
	     mpmod_t modulus, ATTRIBUTE_UNUSED const unsigned long long tmplen, 
	     mpres_t *tmp)
{
  ASSERT (tmplen >= 2);
...
gfp_ext_pow_norm1_sl (mpres_t r0, mpres_t r1, const mpres_t a0, 
                      const mpres_t a1, const long long e, const mpres_t Delta, 
                      mpmod_t modulus, unsigned long long tmplen, mpres_t *tmp)
{
  const unsigned long long abs_e = llabs (e);
  unsigned long long mask = ~0ULL - (~0ULL >> 1);
...
pp1_sequence_g (listz_t g_x, listz_t g_y, mpzspv_t g_x_ntt, mpzspv_t g_y_ntt,
		const mpres_t b1_x, const mpres_t b1_y, const unsigned long long P, 
		const mpres_t Delta, const long long M_param, 
		const unsigned long long l_param, const mpz_t m_1, const long long k_2, 
...
#pragma warning( disable : 4101)  // suppress spurious warning about unused variables
  ...
  mpz_t mt;
#pragma warning( default : 4101)
  unsigned long long i, l = l_param, offset = 0;
  long long M = M_param;
  long long timestart, realstart;
  int want_output = 1;
...
pp1_sequence_h (listz_t h_x, listz_t h_y, mpzspv_t h_x_ntt, mpzspv_t h_y_ntt,
		...
		const long long k_param, const unsigned long long l_param, 
		const unsigned long long P, const mpres_t Delta, 
		mpmod_t modulus_param, const mpzspm_t ntt_context)
{
  unsigned long i;
  __int64 timestart, realstart;
...
#pragma omp parallel if (l_param > 100) private(i)
#endif
  {
    ...
    mpmod_t modulus; /* Thread-local copy of modulus_param */
    mpz_t mt;
    unsigned long long l = l_param, offset = 0;
    long long k = k_param;
...
{
  unsigned long long nr;
  unsigned long long i, l, lenF, lenH, lenG, lenR, tmplen;
  sets_long_t *S_1; /* This is stored as a set of sets (arithmetic 
                       progressions of prime length */
  set_long_t *S_2; /* This is stored as a regular set */
  listz_t F;   /* Polynomial F has roots X^{k_1} for k_1 \in S_1, so has 
  ...
  lenR = nr;
  F = init_list2 ((unsigned int)lenF, (unsigned int) abs (modulus->bits));
  fh_x = init_list2 ((unsigned int)lenF, (unsigned int) abs (modulus->bits));
  fh_y = init_list2 ((unsigned int)lenF, (unsigned int) abs (modulus->bits));
  ...
  g_x = init_list2 ((unsigned int)lenG, (unsigned int) abs (modulus->bits));
  g_y = init_list2 ((unsigned int)lenG, (unsigned int) abs (modulus->bits));
  R_x = init_list2 ((unsigned int)lenR, (unsigned int) abs (modulus->bits));
  R_y = init_list2 ((unsigned int)lenR, (unsigned int) abs (modulus->bits));
  tmplen = 3UL * params->l + list_mul_mem ((unsigned int)params->l / 2) + 20;
  outputf (OUTPUT_DEVVERBOSE, "tmplen = %lu\n", tmplen);
  if (TMulGen_space ((unsigned int)params->l - 1, (unsigned int)params->s_1, 
	  (unsigned int)lenR) + 12 > tmplen)   {
      tmplen = TMulGen_space ((unsigned int)params->l - 1, (unsigned int)params->s_1 - 1, 
  ...
  for (l = 0; l < params->s_2; l++)
    {
      const long long M = params->l - 1 - params->s_1 / 2;
      outputf (OUTPUT_VERBOSE, "Multi-point evaluation %lu of %lu:\n", 
               l + 1, params->s_2);
      ...
      timestart = cputime ();
      if (TMulGen (R_x, (unsigned int)nr - 1, h_x, (unsigned int)params->s_1, g_x, 
		  (unsigned int)params->l - 1, tmp,  modulus->orig_modulus) < 0)
	{
	  outputf (OUTPUT_ERROR, "TMulGen returned error code (probably out "
		   "of memory)\n");
      ...
      timestart = cputime ();
      if (TMulGen (R_y, (unsigned int)nr - 1, h_y, (unsigned int)params->s_1, g_y, 
		  (unsigned int)params->l - 1, tmp,  modulus->orig_modulus) < 0)
	{
	  outputf (OUTPUT_ERROR, "TMulGen returned error code (probably out "
		   "of memory)\n");
	    ...
	    const faststage2_param_t *params, const int twopass)
{
  ...
  F = init_list2 ((unsigned int)lenF, (unsigned int) abs (modulus->bits) + GMP_NUMB_BITS);
  ...
  if (twopass)
    {
      g_y_ntt = g_x_ntt;
      R = init_list2 ((unsigned int)nr, (unsigned int)(mpz_size (modulus->orig_modulus) + 2) *
                          GMP_NUMB_BITS);
    }
  else
