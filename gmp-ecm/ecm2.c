...
multiplyW2n (mpz_t p, point *R, curve *S, mpz_t *q, const unsigned int n, 
              ...
              unsigned __int64 *tot_muls, unsigned __int64 *tot_gcds)
{
...
#endif
      if ((t = (unsigned int)mpz_sizeinbase (q[i], 2) - 1) > maxbit)
          maxbit = t;
    }
...
addWnm (mpz_t p, point *X, curve *S, mpmod_t modulus, unsigned int m, 
        unsigned int n, mpres_t *T, unsigned __int64 *tot_muls, 
        unsigned __int64 *tot_gcds)
{
  unsigned int k, l;
  int i, j;
...
ecm_rootsF (mpz_t f, listz_t F, root_params_t *root_params, 
            unsigned long long dF, curve *s, mpmod_t modulus)
{
  unsigned __int64 i;
  unsigned __int64 muls = 0, gcds = 0;
  __int64 st;
  int youpi = ECM_NO_FACTOR_FOUND;
  listz_t coeffs;
  ...
  if (test_verbose (OUTPUT_VERBOSE))
    {
      ...
      outputf (OUTPUT_VERBOSE,
	       "Initializing tables of differences for F took %lldms",
	       elltime (st, st1));
      outputf (OUTPUT_DEVVERBOSE, ", %llu muls and %llu extgcds", muls, gcds);
      outputf (OUTPUT_VERBOSE, "\n");
      st = st1;
      muls = 0;
      gcds = 0;
  ...
  for (i = 0; i < dF && !youpi;)
    {
      ...
      if (gcd ((unsigned __int64) params->rsieve, 
	       (unsigned __int64) params->dsieve) == 1UL) 
        {
          ...
          if (gcd ((unsigned __int64) params->rsieve, root_params->d1) 
	      == 1UL) 
            mpres_get_z (F[i++], 
...
ecm_rootsG_init (mpz_t f, curve *X, root_params_t *root_params, 
                 unsigned long long dF, unsigned long long blocks, mpmod_t modulus)
{
  ...
  phid2 = (unsigned int)eulerphi (root_params->d2);
...
int 
ecm_rootsG (mpz_t f, listz_t G, unsigned long long dF, ecm_roots_state_t *state, 
            mpmod_t modulus)
{
  unsigned __int64 i;
  unsigned __int64 muls = 0, gcds = 0;
  ...
  outputf (OUTPUT_TRACE, "ecm_rootsG: dF = %llu, state: nr = %u, next = %u, "
