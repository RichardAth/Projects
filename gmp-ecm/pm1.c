            ...
            mpz_t go, int (*stop_asap)(void), char *chkfilename)
{
  ...
  size_n = (unsigned int)mpz_sizeinbase (n->orig_modulus, 2);
  max_size = L1 * size_n;

  mpres_get_z (g, a, n);
  if (mpz_fits_uint_p (g))
    smallbase = (unsigned int)mpz_get_ui (g);
...
static void
print_prob (double B1, const mpz_t B2, unsigned long long dF, unsigned long long k, 
...
pm1 (mpz_t f,  mpz_t p,  mpz_t N,  mpz_t go,  double *B1done,  double B1,
     mpz_t B2min_parm,  mpz_t B2_parm,  unsigned long long k, 
     ...
     gmp_randstate_t rng, int (*stop_asap)(void))
{
  ...
  /* Determine parameters (polynomial degree etc.) */
    ...
    {
      long long P_ntt, P_nontt;
      const unsigned long lmax = 1UL << 30; /* An upper bound */
      unsigned long long lmax_NTT, lmax_noNTT;
      ...
      if (use_ntt)
	{
	  ...
	  outputf (OUTPUT_DEVVERBOSE, "NTT can handle lmax <= %llu\n", lmax_NTT);
      ...
      if (maxmem != 0.)
	{
	  ...
	  outputf (OUTPUT_DEVVERBOSE, "non-NTT can handle lmax <= %llu\n", 
		   lmax_noNTT);
	}
      if (use_ntt != 2)
