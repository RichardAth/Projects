...
C mp_limb_t mulredc10(mp_limb_t * z, const mp_limb_t * x, const mp_limb_t * y,
...
include(`config.m4')
	...
	.section ".opd", "aw"
	.align	3
GSYM_PREFIX`'mulredc10:
	.quad	.GSYM_PREFIX`'mulredc10, .TOC.@tocbase, 0
	.size	GSYM_PREFIX`'mulredc10, 24


C Implements multiplication and REDC for two input numbers of 10 words
...
C local variables: tmp[0 ... 10] array, having 10+1 8-byte words
C The tmp array needs 10+1 entries, but tmp[10] is stored in
C r15, so only 10 entries are used in the stack.
...
C Pass for j = 9. Don't fetch new data from y[j+1].
	...
	addc    r8, r8, r13		C add T0 and low word
	adde    r13, r9, r14		C add high word with carry to T1
	std     r8, 64(r1)		C store tmp[len-2]
	addze   r15, r16		C put carry in r15 (tmp[len] <= 1)
	std     r13, 72(r1)		C store tmp[len-1]
...
C Copy result from tmp memory to z
	...
	.size	.GSYM_PREFIX`'mulredc10, .-.GSYM_PREFIX`'mulredc10
