...
C mp_limb_t mulredc17(mp_limb_t * z, const mp_limb_t * x, const mp_limb_t * y,
...
C local variables: tmp[0 ... 17] array, having 17+1 8-byte words
C The tmp array needs 17+1 entries, but tmp[17] is stored in
...
C Pass for j = 16. Don't fetch new data from y[j+1].
	...
	addc    r8, r8, r13		C add T0 and low word
	adde    r13, r9, r14		C add high word with carry to T1
	std     r8, 120(r1)		C store tmp[len-2]
	addze   r15, r16		C put carry in r15 (tmp[len] <= 1)
	std     r13, 128(r1)		C store tmp[len-1]
