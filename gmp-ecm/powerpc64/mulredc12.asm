...
C mp_limb_t mulredc12(mp_limb_t * z, const mp_limb_t * x, const mp_limb_t * y,
...
GSYM_PREFIX`'mulredc12:
	.quad	.GSYM_PREFIX`'mulredc12, .TOC.@tocbase, 0
	.size	GSYM_PREFIX`'mulredc12, 24


C Implements multiplication and REDC for two input numbers of 12 words
...
C local variables: tmp[0 ... 12] array, having 12+1 8-byte words
C The tmp array needs 12+1 entries, but tmp[12] is stored in
C r15, so only 12 entries are used in the stack.
...
C Pass for j = 11. Don't fetch new data from y[j+1].
	...
	addc    r8, r8, r13		C add T0 and low word
	adde    r13, r9, r14		C add high word with carry to T1
	std     r8, 80(r1)		C store tmp[len-2]
	addze   r15, r16		C put carry in r15 (tmp[len] <= 1)
	std     r13, 88(r1)		C store tmp[len-1]
...
C Copy result from tmp memory to z
	...
	.size	.GSYM_PREFIX`'mulredc12, .-.GSYM_PREFIX`'mulredc12
