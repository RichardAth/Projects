...
C mp_limb_t mulredc7(mp_limb_t * z, const mp_limb_t * x, const mp_limb_t * y,
...
GSYM_PREFIX`'mulredc7:
	.quad	.GSYM_PREFIX`'mulredc7, .TOC.@tocbase, 0
	.size	GSYM_PREFIX`'mulredc7, 24


C Implements multiplication and REDC for two input numbers of 7 words
...
C local variables: tmp[0 ... 7] array, having 7+1 8-byte words
C The tmp array needs 7+1 entries, but tmp[7] is stored in
C r15, so only 7 entries are used in the stack.
...
C Pass for j = 6. Don't fetch new data from y[j+1].
	...
	addc    r8, r8, r13		C add T0 and low word
	adde    r13, r9, r14		C add high word with carry to T1
	std     r8, 40(r1)		C store tmp[len-2]
	addze   r15, r16		C put carry in r15 (tmp[len] <= 1)
	std     r13, 48(r1)		C store tmp[len-1]
...
C Copy result from tmp memory to z
	...
	.size	.GSYM_PREFIX`'mulredc7, .-.GSYM_PREFIX`'mulredc7
