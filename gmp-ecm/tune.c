...
#define MAX_LEN (1ULL << max_log2_len)
...
double x (size_t n)                          \
  { unsigned int __i, __k = 1; long long __st;
...
TUNE_FUNC_START (tune_ntt_mul)
  ...
  TUNE_FUNC_LOOP (ntt_mul (z, x, y, 1LL << n, NULL, 1, mpzspm));
TUNE_FUNC_END (tune_ntt_mul)


TUNE_FUNC_START (tune_list_mul)
  TUNE_FUNC_LOOP (list_mul (z, x, 1LL << n, y, 1LL << n, 1, t));
...
TUNE_FUNC_START (tune_PrerevertDivision)
  TUNE_FUNC_LOOP (PrerevertDivision (z, x, y, 1LL << n, t, mpzspm->modulus));
TUNE_FUNC_END (tune_PrerevertDivision)


TUNE_FUNC_START (tune_ntt_PolyInvert)
  POLYINVERT_NTT_THRESHOLD = 1LL << n;
  
  TUNE_FUNC_LOOP (ntt_PolyInvert (z, x, 1LL << n, t, mpzspm));
TUNE_FUNC_END (tune_ntt_PolyInvert)


TUNE_FUNC_START (tune_PolyInvert)
  
  TUNE_FUNC_LOOP (PolyInvert (z, x, 1LL << n, t, mpzspm->modulus));
...
TUNE_FUNC_START (tune_ntt_polyevalT)
  ...
  POLYEVALT_NTT_THRESHOLD = 1LL << n;

  TUNE_FUNC_LOOP (ntt_polyevalT (z, 1LL << n, Tree, t, mpzspv, mpzspm, NULL));
...
TUNE_FUNC_START (tune_polyevalT)
  ...
  TUNE_FUNC_LOOP (polyeval_tellegen (z, 1LL << n, Tree, t, 3 * (1 << n),
...
TUNE_FUNC_START (tune_mpzspv_normalise)
  MPZSPV_NORMALISE_STRIDE = 1LL << n;
  
  TUNE_FUNC_LOOP (mpzspv_normalise (mpzspv, 0,
    1LL << MAX_LOG2_MPZSPV_NORMALISE_STRIDE, mpzspm));
...
main (int argc, char **argv)
{
  ...
  MUL_NTT_THRESHOLD = 1LL << crossover2 (tune_list_mul, tune_ntt_mul, 1,
      max_log2_len, 2);

  printf ("#define MUL_NTT_THRESHOLD %lu\n", (unsigned long) MUL_NTT_THRESHOLD);

  PREREVERTDIVISION_NTT_THRESHOLD = 1LL << crossover2 (tune_PrerevertDivision,
