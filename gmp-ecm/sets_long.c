...
{
  unsigned long i;
  const unsigned long long c = S->card; /* We might overwrite S->card */
...
static inline void
swap_long (long long *a, long long *b)
{
  long long t;
  t = *a;
  *a = *b;
...
static inline void 
swapsort_long (long long *a, long long *b)
{
  if (*a > *b)
    swap_long (a, b);
}
...
void 
quicksort_long (long long *a, unsigned long long l)
{
...
static void
sets_max_recurse (mpz_t S, const unsigned long long beta)
{
  unsigned long long P = beta, p, pk;
  unsigned int k;
...
void
sets_max (mpz_t S, const unsigned long long beta)
{
  mpz_set_ui (S, 0UL);
...
sets_sumset_recurse (long long *sum, const set_long_t *sets, 
                    const unsigned long long nr_sets, const long long add)
{
...
sets_factored_Rn2 (set_long_t **L, size_t *sets_size, const long long n, 
                   const long long k)
{
...
{
  unsigned long long r, k, nr = 0ULL;
  long long p, np;
...
{
  unsigned long long i, nr_unsorted, highest_swap;
  set_long_t *set;
...
{
  unsigned long long i, c, remaining_d = d;
  set_long_t *readfrom, *readnext, *moveto, *extractto = NULL;
  size_t extracted_size = sizeof (unsigned long long);
  ...
  if (d == 1UL)
    {
      ...
      if (extr_size != NULL)
        *extr_size = sizeof (unsigned long long) + set_sizeof (1ULL);
      return;
    }
...
sets_long_t *
sets_get_factored_sorted (const unsigned long long beta)
{
  sets_long_t *sets;
  size_t size;
