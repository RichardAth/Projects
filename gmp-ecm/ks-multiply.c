  ...
  for (i = 0, j = 0; i < l; i++, t += s, j += stride)
    {
      ...
      SIZ(R[j]) = (int)size_tmp;
    }
}

/* R <- A * B where A = A[0] + A[1]*x + ... + A[n-1]*x^(n-1), idem for B */
void
list_mul_n_basecase (listz_t R, listz_t A, listz_t B, unsigned long long n)
{
  unsigned int i, j;
...
static void
list_mul_n_karatsuba_aux (listz_t R, listz_t A, listz_t B, unsigned long long n,
                          listz_t T)
{
...
list_mul_n_mem (unsigned long long n)
{
  if (n == 1)
    return 0;
  else
...
void
list_mul_n_karatsuba (listz_t R, listz_t A, listz_t B, unsigned long long n)
{
  listz_t T;
...
void
list_mul_n_KS1 (listz_t R, listz_t A, listz_t B, unsigned long long l)
{
...
*/
void
list_mul_n_KS2 (listz_t R, listz_t A, listz_t B, unsigned long long n)
{
  unsigned long long i;
...
*/
void
list_mult_n (listz_t R, listz_t A, listz_t B, unsigned long long n)
{
...
int
TMulKS (listz_t b, unsigned long long n, listz_t a, unsigned long long m,
        listz_t c, unsigned long long l, mpz_t modulus, int rev)
{
  unsigned long long i, s = 0, t;
  mp_ptr ap, bp, cp;
  mp_size_t an, bn, cn;
...
unsigned int
ks_wrapmul_m (unsigned long long m0, unsigned long long k, mpz_t n)
{
 #ifdef FFT_WRAP
	mp_size_t t, s;
	unsigned long i, m;
...
*/
unsigned int
ks_wrapmul (listz_t R, unsigned long long m0,
            listz_t A, unsigned long long k,
            listz_t B, unsigned long long l,
	    mpz_t n)
{
#ifndef FFT_WRAP
