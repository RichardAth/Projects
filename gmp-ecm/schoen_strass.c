...
static void 
F_mul_sqrt2exp (mpz_t R, mpz_t S, long long e, unsigned int n) 
{
  int chgsgn = 0, odd;
...
#define A2s A[2LL * l << stride2]
#define A3s A[3LL * l << stride2]
#define A0is A[(long long)i << stride2]
#define A1is A[(long long)(i + l) << stride2]
#define A2is A[(long long)(i + 2 * l) << stride2]
#define A3is A[(long long)(i + 3 * l) << stride2]
...
/* Decimation-in-frequency FFT. Unscrambled input, scrambled output.
   ...
   Assumes l > 1. */
static void 
F_fft_dif (mpz_t *A, long long l, int stride2, int n) 
{
	long long i;
	long long omega = (4 * n) / l, iomega;
  ...
  if (l > 1)
    {
      ...
      F_fft_dif (A + ((long long)l << stride2), l, stride2, n);
      F_fft_dif (A + (2 * (long long)l << stride2), l, stride2, n);
      F_fft_dif (A + (3 * (long long)l << stride2), l, stride2, n);
    }
}
...
/* Decimation-in-time inverse FFT. Scrambled input, unscrambled output.
   ...
   Assume l > 1. */
static void 
F_fft_dit (mpz_t *A, long long l, int stride2, int n) 
{
  long long i, omega = (4 * n) / l, iomega;
...
F_toomcook4 (mpz_t *C, mpz_t *A, mpz_t *B, unsigned long long len, unsigned int n, 
             mpz_t *t)
{
  unsigned long long l, i, r;
...
static unsigned long long
F_karatsuba (mpz_t *R, mpz_t *A, mpz_t *B, unsigned long long len, unsigned int n, 
             mpz_t *t)
{
...
*/
unsigned long long 
F_mul (mpz_t *R, mpz_t *A, mpz_t *B, unsigned long long len, int parameter, 
       unsigned int n, mpz_t *t)
{
  unsigned long long i, r=0;
  unsigned long long transformlen = (parameter == NOPAD) ? len : 2 * len;
#ifdef CHECKSUM
...
unsigned long long
F_mul_trans (mpz_t *R, mpz_t *A, mpz_t *B, unsigned long long lenA, 
             unsigned long long lenB, unsigned int n, mpz_t *t)
{
  unsigned long long i, r = 0, len2;
  ...
  if (lenB <= 4 * n)
    {
      ...
      unsigned long long h = lenB / 4;
      const unsigned long long lenA0 = h, lenA1 = lenA - h;
