...
mpz_set_uint64 (mpz_t m, const uint64_t n)
{
  ...
  if (sizeof(mp_limb_t) > sizeof(unsigned __int64))
    {
       mpz_set_ui (m, (unsigned __int64)(uint32_t)(n >> 32));
       mpz_mul_2exp (m, m, 32);
       mpz_add_ui (m, m, (unsigned __int64)(uint32_t)n);
    }
  else
    mpz_set_ui(m, (unsigned __int64)n);
