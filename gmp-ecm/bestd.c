...
int
bestD (root_params_t *root_params, unsigned long long *finalk, 
       unsigned long long *finaldF, mpz_t B2min, mpz_t B2, int po2, int use_ntt, 
...
#define Npo2 23
  ...
  unsigned long long i, d1 = 0, d2 = 0, dF = 0, phid, k, maxN;
  mpz_t j, t, i0, i1;
  int r = 0;
    ...
    for (i = 0; i < maxN; i++)
      {
        ...
        d1 = (po2) ? lpo2[i] : l[i];
        phid = eulerphi (d1) / 2;
        dF = (po2) ? 1ULL << ceil_log2 (phid) : phid;
        lg_dF = ceil_log2 (dF);
        
        if (use_ntt)
          sp_num = (int)(2 * mpz_sizeinbase (modulus->orig_modulus, 2) + lg_dF) / 
                 SP_NUMB_BITS + 4;
