...
typedef struct
{
  ...
  unsigned __int64 magic;	/* used for debugging purposes while writing this code */
#endif
  ...
  size_t nexprlen;	/* strlen of expression, 0 if there is NO expression */
...
int write_resumefile (char *, int, mpz_t, ecm_params params,
		      ...
		      const char *);
long long write_s_in_file (char *, mpz_t);
int read_s_from_file (mpz_t, char *, double); 

/* main.c */
int kbnc_z (double *k, unsigned __int64 *b, unsigned __int64 *n, signed __int64 *c,
            mpz_t z);
int kbnc_str (double *k, unsigned __int64 *b, unsigned __int64 *n, signed __int64 *c,
              char *z, mpz_t num);
