// QuadraticModularEquationSolver.cpp : Defines the functions for the static library.
//
...
static Znum ValCOdd;
static Znum Mult;
static Znum currentSolution;
static Znum discriminant;
static Znum SqrtDisc;
Znum prime;
static Znum bigBase;
static Znum K1;
static Znum L;
...
struct sFactors astFactorsMod[MAX_FACTORS];


/* result = mult * factor */
void multint(Znum* pResult, const Znum* pMult, int factor) {
    *pResult = *pMult * factor;
}

/* Sum = Addend1 + Addend2 */
void BigIntAdd(const Znum* pAddend1, const Znum* pAddend2, Znum* pSum) {
    *pSum = *pAddend1 + *pAddend2;
}
/* difference = Minuend - subtrahend */
void BigIntSubt(const Znum* pMinuend, const Znum* pSubtrahend, Znum* pDifference) {
    *pDifference = *pMinuend - *pSubtrahend;
}

/* Dest = Src */
void CopyBigInt(Znum* pDest, const Znum* pSrc) {
    if (pDest != pSrc)
        *pDest = *pSrc;
}

void IntArray2BigInteger(const long long* ptrValues, Znum* bigint) {
    const long long* piValues = ptrValues;
    int sign;
    long long nbrLimbs = *piValues;
    piValues++;
...
/* Power = Base^exponent */
void BigIntPowerIntExp(const Znum* pBase, int exponent, Znum* pPower) {
    mpz_pow_ui(pPower->backend().data(), pBase->backend().data(), exponent);
}
...
/* inv = num^(-1) mod M.                                               */
/********************************************************************* */
bool ModInvBigNbr(const Znum* num, Znum* inv, const Znum* mod) {
    return mpz_invert(inv->backend().data(), num->backend().data(), mod->backend().data());
}

void BigIntModularDivision(const Znum* Num, const Znum* Den,
    const Znum* mod, Znum* quotient) {
    Znum invDen;
    bool rv = ModInvBigNbr(Den, &invDen, mod);
    assert(rv);
    *quotient = *Num * invDen;
    mpz_mod(quotient->backend().data(), quotient->backend().data(), mod->backend().data());
}

/* remainder = Dividend% Divisor */
void BigIntRemainder(const Znum* pDividend, const Znum* pDivisor, Znum* pRemainder) {
    mpz_fdiv_r(pRemainder->backend().data(), pDividend->backend().data(), pDivisor->backend().data());
}

/* product = Fact1 * Fact2 */
void BigIntMultiply(const Znum* pFact1, const Znum* pFact2, Znum* pProduct){
    *pProduct = *pFact1 * *pFact2;
}

/* Result = iMult * Mult + addend */
void multadd(Znum *pResult, long long iMult, const Znum* pMult, long long addend) {
    *pResult = *pMult * iMult + addend;
}
...
void PerformChineseRemainderTheorem(void)
{
    ...
    do
    {
        ...
        for (T1 = nbrFactors - 1; T1 >= 0; T1--)
        {
            ...
            if  (K1.is_zero()) //((K1.nbrLimbs == 1) && (K1.limbs[0].x == 0))
...
//static void setNbrLimbs(Znum* pBigNbr)
//{
...
// Solve Bx + C = 0 (mod N).
static void SolveModularLinearEquation(Znum* pValA, const Znum* pValB,
    const Znum* pValC, Znum* pValN)
{
    ...
    Znum* ptrSolution1 = common.quad.Solution1;
    Znum* ptrSolution2 = common.quad.Solution2;
...
static void findQuadraticSolution(Znum* pSolution, int exponent)
{
    int bytesLen;
    int expon = exponent;
    int bitMask = 1;
...
static bool SolveQuadraticEqModPowerOf2(int exponent, int factorIndex,
    const Znum* pValA, const Znum* pValB, const Znum* pValC)
{
    int expon = exponent;
    int bitsAZero;
    int bitsBZero;
...
static bool SolveQuadraticEqModPowerOfP(int expon, int factorIndex,
    const Znum* pValA, const Znum* pValB)
{
    int correctBits;
    int nbrLimbs;
    int bitsAZero;
    ...
    const Znum* pValA, const Znum* pValB, const Znum* pValC)
{
    ...
    Znum* ptrSolution = &common.quad.Solution1[factorIndex];
    int NumberLengthBytes;
    NumberLength = prime.nbrLimbs;
    NumberLengthBytes = NumberLength * (int)sizeof(limb);
    (void)memcpy(TestNbr, prime.limbs, NumberLengthBytes);
...
void SolveEquation(Znum* pValA, const Znum* pValB,  const Znum* pValC, 
    Znum* pValN,  Znum* pGcdAllParm, Znum* pValNnParm)
{
    ...
    pGcdAll = pGcdAllParm;
    pValNn = pValNnParm;
    BigIntRemainder(pValA, pValN, &Aux[0]);
    if (Aux[0].is_zero()) //(BigIntIsZero(&Aux[0]))
    {           // Linear equation.
    ...
    {     // Last modulus is different from ValN.
        ...
        //NumberLength = pValN->nbrLimbs;
        //nbrToFactor = *pValN; //BigInteger2IntArray(nbrToFactor, pValN);
        ...
        factorise(*pValN,  factorsMod, nullptr);
    }
    Q = 0; //intToBigInteger(&Q, 0);
    nbrFactors = factorsMod.fsize();
    pstFactor = factorsMod.f;
