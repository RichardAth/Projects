...
1. An assembly module  Int128x64.asm that implements 128 bit arithmetic.
   The C/C++ declarations for them are:
	...
	NB this is compiled using MASM, not vyasm!
    before compiling this, go to Solution Explorer window, right-click on project,
    select Build Dependencies > Build Customisations and check that the masm box 
    is ticked.
    Then right-click on the element Int128x64.asm, click properties and  check
    that the type is Microsoft Macro Assembler.

2. A header file Int128.h that creates classes for signed and unsigned 128 bit integers
...
3. File Int128Str.cpp contains the following functions:
