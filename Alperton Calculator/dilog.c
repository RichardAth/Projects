...
@alpertron alpertron Added more functions to expression evaluator, always use batch mode i…
903 lines (882 sloc) 34 KB */
/*
...
static void indicateCannotComputeLog(int indexBase, int indexExp)
{
	...
	BigInteger2Dec(&tmpBase, textExp + strlen(textExp), groupLen);
...
void DiscreteLogarithm(void)
{
	...
	if (!TestBigNbrEqual(&LastModulus, &modulus))
	{
		...
		BigInteger2Dec(&modulus, tofactorDec, groupLen);
	...
	for (index = 1; index <= NbrFactorsMod; index++)
	{
		...
		for (indexBase = 0; indexBase < NbrFactors; indexBase++)
		{
			...
			BigInteger2Dec(&subGroupOrder, textExp + strlen(textExp), groupLen);
...
void dilogText(char *baseText, char *powerText, char *modText, int groupLength)
{
	...
	{
		strcpy(ptrOutput, lang ? "\nHallar <var>exp</var> tal que " :
			"\nFind <var>exp</var> such that ");
		ptrOutput += strlen(ptrOutput);
		//Bin2Dec(base.limbs, base.nbrLimbs);
		BigInteger2Dec(&base, ptrOutput, groupLength);
		ptrOutput += strlen(ptrOutput);

		strcat(ptrOutput, "<sup><var>exp</var></sup> &equiv; ");
		ptrOutput += strlen(ptrOutput);
		//Bin2Dec(power.limbs, ptrOutput, power.nbrLimbs, groupLength);
		BigInteger2Dec(&power, ptrOutput, groupLength);
		ptrOutput += strlen(ptrOutput);

		strcat(ptrOutput, " (mod ");
		ptrOutput += strlen(ptrOutput);
		//Bin2Dec(modulus.limbs,  modulus.nbrLimbs, groupLength);
		BigInteger2Dec(&modulus, ptrOutput, groupLength);
		ptrOutput += strlen(ptrOutput);
		strcat(ptrOutput, ")\n\n");
		ptrOutput += strlen(ptrOutput);
		...
		{
			strcat(ptrOutput, lang ? "Ningún valor de <var>exp</var> satisface la congruencia.\n" :
				"There is no such value of <var>exp</var>.\n");
			ptrOutput += strlen(ptrOutput);
		...
		else
		{
			...
			BigInteger2Dec(&DiscreteLog, ptrOutput, groupLength);
			ptrOutput += strlen(ptrOutput);
			...
			{   // Discrete log period is not zero.
				...
				BigInteger2Dec(&DiscreteLogPeriod, ptrOutput, groupLength);
				ptrOutput += strlen(ptrOutput);
