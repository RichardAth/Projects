/* calculators/expression.c
@alpertron alpertron Added XOR, SHL and SHR operators. Added hex output for some apps.
1416 lines (1380 sloc) 38.4 KB */
/*
...
int lang;					// defines language; English (0) or Spanish (!= 0)
char output[3000000];       // output buffer; allows for 3 million characters
limb Mult1[MAX_LEN];
limb Mult3[MAX_LEN];
limb Mult4[MAX_LEN];
int q[MAX_LEN];
...
enum eExprErr ComputeExpression(char *expr, int typ, BigInteger *ExpressionResult)
{
	...
	stackIndex = 0;
	exprIndex = 0;
	exprType = typ;           // doesn't seem to be used anywhere
...
static enum eExprErr ComputeExpr(char *expr, BigInteger *ExpressionResult)
{
	...
	while (exprIndex < exprLength)
	{
		...
		charValue = *(expr + exprIndex);  // get next character of the expression

		/* find operator and replace charvalue with OPER_<code>
		or evaluate number
...
/* Check whether expr matches the specified function name. If so,
compute value of parameters. If not, return value 1*/
static int func(char *expr, BigInteger *ExpressionResult,
	char *funcName, int funcArgs, int leftNumberFlag)
{
	int index;
	...
	{
		return 1;   // function name longer expr, so name cannot match
	}
	...
	while (*ptrFuncName)
	{
		...
		{
			return 1;  // FuncName does not match name in expression 
		}
		ptrExpr++;
		ptrFuncName++;
	}
	exprIndex += funcNameLen;  // move index past function name 
	if (leftNumberFlag == 1)
	{
		return EXPR_SYNTAX_ERROR;
	}
...
/*  F(n) Fibonnaci number (origValue = 0)
L(n): Lucas number Ln = Fn-1 + Fn+1 (origValue = 2)*/
static enum eExprErr ComputeFibLucas(int origValue)
{
	BigInteger *pArgument = &stackValues[stackIndex];
	limb largeVal;
...
/* convert number toFactor to ascii, also factorise number.
Ascii text is left in tofactorDec, */
void PerformFactorization(const BigInteger *tofactor)
{
	...
	if (hexadecimal)
	{
		...
		BigInteger2Hex(tofactor, tofactorDec, groupLen);
	}
	else
	{
		/*Bin2Dec(tofactor->limbs, tofactorDec, tofactor->nbrLimbs, groupLen);*/
		BigInteger2Dec(tofactor, tofactorDec, groupLen);
	}
