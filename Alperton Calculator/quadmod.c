...
/* uses global variable ptrOutput */
...
/* uses global variable ptrOutput */
static void Show1(BigInteger *num, int t)
{
	int u = Show(num, "", t);
...
/* uses global variable ptrOutput */
static void Solution(BigInteger *value)
{
	SolNbr++;
...
static void SolveEquation(void)
{
	...
	{        // Mod 0 => Equation in integer numbers
		...
		{      // Not Quadratic Equation
			...
			{    // Constant Equation
				...
				{  // 0 = 0
					strcpy(ptrOutput, "\nThe equation is satisfied by any integer <var>x</var>.\n");
					ptrOutput += strlen(ptrOutput);
				}
				else
				{
	...
	{     // All values from 0 to GcdAll - 1 are solutions.
		...
		{
			strcpy(ptrOutput, "\nAll values of <var>x</var> between 0 and ");
			ptrOutput += strlen(ptrOutput);
			addbigint(&GcdAll, -1);
			BigInteger2Dec(&GcdAll, ptrOutput, groupLen);
			ptrOutput += strlen(ptrOutput);
			strcpy(ptrOutput, " are solutions.\n");
			ptrOutput += strlen(ptrOutput);
		}
		else
		{
	...
	{     // Last modulus is different from ValN.
		...
		BigInteger2Dec(&ValN, tofactorDec, groupLen);
...
static void textErrorQuadMod(char *pOutput, enum eExprErr rc)
{
	textError(pOutput, rc);
}

/* this is the entry point to all functions in this module */
void quadmodText(char *quadrText, char *linearText, char *constText, char *modText, int groupLength)
{
	int u;
	char *ptrBeginSol;
	enum eExprErr rc;
	ptrOutput = output;   // copy output pointer to variable that's local to this module
	strcpy(ptrOutput, "2\n");
