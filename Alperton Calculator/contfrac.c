...
static void ContFrac(void)
{
	...
	strcpy(ptrOutput, lang ? " la fracción cuyo numerador es </span>" : 
	...
	{   /* Complex number */
		showText(lang != 0 ? "\nEl número no es real, por lo que no tiene desarrollo en fracciones continuas.\n" :
	...
	else
	{
		...
		{  // Too many convergents.
			showText(lang != 0 ? ", ... </span>//<br />donde la parte periódica (truncada a partir de los 100000 convergentes) está señalada en negrita.\n" :
