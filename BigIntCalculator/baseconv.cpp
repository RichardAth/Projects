...
static void Bin2Dec(const limb binary[], char* decimal, int nbrLimbs, int groupLength)
{
...
/* convert BigInt to ascii digits in Decimal, add terminating null, move pDecimal past digits */
void BigInteger2Dec(char** ppDecimal, const BigInteger* pBigInt, int groupLength)
{
    ...
    Bin2DecV2(&ptrDecimal, pBigInt->limbs, pBigInt->nbrLimbs, groupLength);
...
std::ostream  &operator<<(std::ostream  &s, const BigInteger  &n) {
    ...
    double buflen = (double)length*std::log10(2);  // get length in decimal digits
