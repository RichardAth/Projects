...
static std::vector<Znum>ModSqrtp2x(const Znum& c, const Znum& prime, const int lambda) {
    ...
    long long k = countZeroBits(c);  /* c = 2^k */
    assert((k & 1) == 0);  /* k must be even */

    sqrtc = power((Znum)2, k / 2);   /* get sqrt(c) */
...
static std::vector <Znum> ModSqrt2xs(const Znum& c, const Znum& prime, const int lambda) {
    ...
    long long k = extract(c, prime);   /* find k such that prime^k = x */
    assert((k & 1) == 0);   /* k must be even */
...
#undef min  /* remove min defined in windows.h */
    ...
    rx = primeModSqrt(c, prime);   /* rx^2 ≡ x (mod prime) */
    if (rx.empty()) {
        roots.clear();
        printroots(c, mod, roots);
    ...
    if ((root * root) % mod == c) {
        ...
        if (verbose > 1 || root > 0) {
            std::cerr << "discarded invalid root: x = " << c << " mod = " << mod << " root = " << root << '\n';
        }
    }
    printroots(c, mod, roots);
    return roots;
...
/* find r such that r^2 ≡ x mod prime^lambda, where lambda > 1,
  the method used was partly derived from Wikipedia but the cases where x=0,
  ...
  believe me, I looked.
  There are several different methods used, depending the values of x and the modulus:
  1. x ≡ 0
  2  gcd(x, prime^lambda) > 1 and is a perfect square, prime = 2
  3  gcd(x, prime^lambda) > 1 and is a perfect square, prime > 2
  4  gcd(x, prime^lambda) > 1 and is not a perfect square (there are no solutions)
  5  gcd(x, prime^lambda) = 1, prime = 2
  6. gcd(x, prime^lambda) = 1, prime > 2 */
static std::vector <Znum> ModSqrt2(const Znum& xx, const Znum& prime, const int lambda) {
    Znum mod = power(prime, lambda);
    Znum x, r1, r2, gcdv, sqrtgcd;
    std::vector <Znum> roots;

    x = xx % mod;
    if (x < 0)
        x += mod;  /* ensure x is in range 0 to mod-1 */

    /* treat x=0 as special case. 0 is a solution, but there may be others. */
    if (x == 0) {
        /* case 1 */
        ...
        printroots(x, mod, roots);
        return roots;
    }

    /* x > 0 */
    gcdv = gcd(x, mod);    /* get gcd of x and mod */
    if (gcdv > 1) {
        if (isPerfectSquare(gcdv, sqrtgcd)) {
            if (prime == 2)
                /* case 2:  gcd(x, prime^lambda) > 1 and is a perfect square, prime = 2 */
                roots = ModSqrtp2(x/gcdv, prime, lambda);  //  Find modular square root, modulus = prime^lambda
            else
                /* case 3:  gcd(x, prime^lambda) > 1 and is a perfect square, prime > 2 */
                roots = ModSqrt2x(x / gcdv, prime, lambda);  //  Find modular square root, modulus = prime^lambda
            if (roots.empty())
        ...
        else {   /* case 4: gcd(x, prime^lambda) > 1 and is not a perfect square (there are no solutions) */
            printroots(x, mod, roots);
            return roots;  /* gcdv is not a perfect square: no solutions*/
        }
    }
    else  /* x and mod are mutually prime*/
        if (prime == 2)
            /* case 5: gcd(x, prime^lambda) = 1, prime = 2 */
            return ModSqrtp2(x, prime, lambda); //  Find modular square root of x, modulus = prime^lambda
        else
            /* case 6: gcd(xc prime^lambda) = 1, prime > 2 */
            return(ModSqrt2x(x, prime, lambda));  //  Find modular square root of x, modulus = prime^lambda
}
...
static long long ZandInt(const Znum a, long long x) {
    ...
    if (sz == 0)
        return 0;
    last = ZT(a)->_mp_d[0];  /* get least significant 64 bits of a */
    last &= x;
    return last;
}
...
Legendre or Lagrange method is used rather than Tonelli-Shanks if either is 
applicable, depending on the value of prime.
...
std::vector <Znum> primeModSqrt(const Znum &aa, const Znum &prime) {
    ...
    case 1: /* Tonelli-Shanks */ {
        ...
        while (t != 1) {
            ...
            b = modPower(c, pow2bi(temp), prime); /* b = x^(2^(m-i-1)) mod prime */
