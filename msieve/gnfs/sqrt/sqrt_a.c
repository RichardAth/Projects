...
static uint32 verify_product(mpz_poly_t *gmp_prod, abpair_t *abpairs, 
	...
	uint32 i, j;
	uint64 c_mod_q = mpz_fdiv_ui(c, q);
	...
	for (i = 0; i <= d; i++) {
		prod[i] = 0;
		ref_prod[i] = (uint32)mpz_fdiv_ui(gmp_prod->coeff[i],
					(unsigned long)q);
		mod[i] = (uint32)mpz_fdiv_ui(alg_poly->coeff[i],
					(unsigned long)q);
	}
	prod[0] = 1;
	...
	for (i = 0; i < num_relations; i++) {
		...
		if (a < 0)
			a += q;
		ac = mp_modmul_1((uint32)a, (uint32)c_mod_q, q);
...
static uint32 get_initial_inv_sqrt(msieve_obj *obj, mpz_poly_t *alg_poly,
	...
	for (i = 0; i < ISQRT_NUM_ATTEMPTS; i++) {
		if (get_prime_for_sqrt(alg_poly, (uint32)start_q + 1, (uint32*)&q)) {
			if (start_q > 150) {
		...
		if (inv_sqrt_mod_q(isqrt_mod_q, prod, 
				alg_poly, (uint32)q, &obj->seed1, &obj->seed2)) {
			break;
		}
		start_q = q;
	}
...
void alg_square_root(msieve_obj *obj, mpz_poly_t *alg_poly, 
	...
	i = (uint32)mpz_size(prod.coeff[0]);
