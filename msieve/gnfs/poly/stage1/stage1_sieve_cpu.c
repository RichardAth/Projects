		...
		uint64 special_q_root, uint64 block_size, uint64 *inv_array)
{
	...
	int64 sieve_start = -(int64)sieve_size;
	uint32 num_blocks = 0;
		...
		uint32 p_min, uint32 p_max, double deadline, double *elapsed)
{
	...
	hashtable_size_log2 = (uint32)ceil(log(calc_hashtable_size * 5. / 3.) / M_LN2);
	hashtable = (hash_entry_t *)xmalloc(sizeof(hash_entry_t) *
				((uint64)1 << hashtable_size_log2));
	...
	while (1) {
		...
		for (i = 0, tmp = hash_array.packed_array; i < num_p; i++) {
			...
			if (mp_gcd_1((uint32)mpz_tdiv_ui(qprod, tmp->p), tmp->p) == 1)
		...
		poly_coeff_t *c, double deadline)
{
	...
	p_min = (uint32)MIN(MAX_OTHER / P_SCALE, sqrt(2.5 / sieve_bound));
	p_min = (uint32)MIN(p_min, pow((double)SIEVE_MAX / sieve_bound, 1. / 4.) - 1);
	p_min = (uint32)MIN(p_min, sqrt(p_size_max) / P_SCALE);

	p_max = (uint32)(p_min * P_SCALE);

	special_q_min = 1;
	special_q_max = (uint32)MIN(MAX_SPECIAL_Q, p_size_max / p_max / p_max);
	...
	num_pieces = (uint32)MIN(450, (double)special_q_max * p_max
	...
	for (i = 0; i < num_passes; i++) {
		uint32 quit;
		double elapsed = 0;
		int64 sieve_size = (int64)MIN(sieve_bound * pow((double)p_min, 4.),
					SIEVE_MAX);
		...
		p_min = p_max;
		//p_max *= P_SCALE;
		p_max = (uint32)(p_max * P_SCALE);

		//special_q_max /= (P_SCALE * P_SCALE);
		special_q_max = (uint32)(special_q_max / (P_SCALE * P_SCALE));
	}
