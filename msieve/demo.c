...
/* the 1st paraneter contains the value SIGFPE,
...
void handle_fpe(int sig, int num) {
	...
	__debugbreak();     // try to enter debuggger (look at call stack)
	return;
}
...
int main(int argc, char **argv) {
	...
	if (signal(SIGFPE, (void(__cdecl *)(int))handle_fpe) == SIG_ERR) {
		printf("could not install handler on SIGFPE\n");
		return -1;
	}

	err = _controlfp_s(&control_word, _EM_INEXACT | EM_UNDERFLOW, MCW_EM);  
	/* allow hardware FP exceptions exept inexact */
	//err = _controlfp_s(&control_word, 0, MCW_EM);  /* allow hardware FP exceptions */
		
	if (err) {
		printf("could not set FP control word\n");
		return -1;
	}
#ifdef HAVE_MPI
	{
				...
				if (i + 1 < argc && argv[i+1][0] != '-') {
					if (tolower(argv[i][1]) == 'i')
						...
						if((p=strstr(savefile_name, ".gz"))) {
							savefile_name = _strdup(argv[i+1]);
	...
	if (isdigit(buf[0]) || buf[0] == '(' ) {
		...
		while (1) {
			...
			clock_t start = clock();	// used to measure execution time
			factor_integer(buf, flags, savefile_name, 
					logfile_name, nfs_fbfile_name,
					&seed1, &seed2,
					max_relations, 
			...
			clock_t end = clock();              // measure amount of time used
			double elapsed = (double)(end - start);
			if (elapsed / CLOCKS_PER_SEC > 10)
				printf_s ("** Completed: time used= %g seconds \n", elapsed / CLOCKS_PER_SEC);
			if (feof(stdin))
				break;
		}
	}
