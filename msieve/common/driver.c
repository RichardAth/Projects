...
void logprintf(const msieve_obj *obj, char *fmt, ...) {
	...
	if (obj->flags & MSIEVE_FLAG_USE_LOGFILE) {
		...
		va_start(ap, fmt);
		buf[0] = 0;
		strcpy(buf, ctime(&t));    // timestamp in format: Mon Jul 16 02:03:55 1987\n\0
		*(strchr(buf, '\n')) = 0;  // remove newline from buf
		fprintf(logfile, "%s  ", buf);
		vfprintf(logfile, fmt, ap);  // print variable log text
		fclose(logfile);
		va_end(ap);
	}
	if (obj->flags & MSIEVE_FLAG_LOG_TO_STDOUT) {
